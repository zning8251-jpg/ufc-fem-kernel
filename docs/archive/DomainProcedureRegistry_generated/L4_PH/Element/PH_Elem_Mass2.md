...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L4_PH/Element/PH_Elem_Mass2.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../05_Project_Planning/PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
```fortran
  ...
  CONTAINS
    PROCEDURE, PUBLIC :: Init => PH_Elem_Mass_Algo_Init
    PROCEDURE, PUBLIC :: Valid => PH_Elem_Mass_Algo_Valid
...
```fortran
  ...
  CONTAINS
    PROCEDURE, PUBLIC :: Init => PH_Elem_Mass_State_Init
    PROCEDURE, PUBLIC :: Clear => PH_Elem_Mass_State_Clear
    PROCEDURE, PUBLIC :: Print => PH_Elem_Mass_State_Print
...
|------|------|------|-------------------------|
| SUBROUTINE | `PH_Elem_Mass_Algo_Init` | 107 | `SUBROUTINE PH_Elem_Mass_Algo_Init(this, density, mass_formulation, &` |
| SUBROUTINE | `PH_Elem_Mass_Algo_Valid` | 142 | `SUBROUTINE PH_Elem_Mass_Algo_Valid(this, status)` |
| SUBROUTINE | `PH_Elem_Mass_State_Init` | 174 | `SUBROUTINE PH_Elem_Mass_State_Init(this, n_dofs, mass_type, status)` |
| SUBROUTINE | `PH_Elem_Mass_State_Clear` | 196 | `SUBROUTINE PH_Elem_Mass_State_Clear(this)` |
| SUBROUTINE | `PH_Elem_Mass_State_Print` | 210 | `SUBROUTINE PH_Elem_Mass_State_Print(this, status)` |
