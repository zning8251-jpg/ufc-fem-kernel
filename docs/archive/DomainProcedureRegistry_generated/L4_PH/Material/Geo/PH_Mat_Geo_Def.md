...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L4_PH/Material/Geo/PH_Mat_Geo_Def.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../../05_Project_Planning/PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
```fortran
  ...
  CONTAINS
    PROCEDURE, PASS :: Init  => PH_Mat_Geo_Desc_Init
    PROCEDURE, PASS :: Valid => PH_Mat_Geo_Desc_Valid
    PROCEDURE, PASS :: Clean => PH_Mat_Geo_Desc_Clean
...
```fortran
  ...
  CONTAINS
    PROCEDURE, PASS :: Init   => PH_Mat_Geo_State_Init
    PROCEDURE, PASS :: Update => PH_Mat_Geo_State_Update
    PROCEDURE, PASS :: Clean  => PH_Mat_Geo_State_Clean
...
```fortran
  ...
  CONTAINS
    PROCEDURE, PASS :: Init   => PH_Mat_Geo_Algo_Init
    PROCEDURE, PASS :: Config => PH_Mat_Geo_Algo_Config
...
```fortran
  ...
  CONTAINS
    PROCEDURE, PASS :: Init  => PH_Mat_Geo_Ctx_Init
    PROCEDURE, PASS :: Clean => PH_Mat_Geo_Ctx_Clean
...
|------|------|------|-------------------------|
| SUBROUTINE | `PH_Mat_Geo_Desc_Init` | 108 | `SUBROUTINE PH_Mat_Geo_Desc_Init(desc, sub_type, nprops)` |
| FUNCTION | `PH_Mat_Geo_Desc_Valid` | 118 | `FUNCTION PH_Mat_Geo_Desc_Valid(desc) RESULT(valid)` |
| SUBROUTINE | `PH_Mat_Geo_Desc_Clean` | 124 | `SUBROUTINE PH_Mat_Geo_Desc_Clean(desc)` |
| SUBROUTINE | `PH_Mat_Geo_State_Init` | 138 | `SUBROUTINE PH_Mat_Geo_State_Init(state)` |
| SUBROUTINE | `PH_Mat_Geo_State_Update` | 150 | `SUBROUTINE PH_Mat_Geo_State_Update(state, stress, plastic_strain)` |
| SUBROUTINE | `PH_Mat_Geo_State_Clean` | 158 | `SUBROUTINE PH_Mat_Geo_State_Clean(state)` |
| SUBROUTINE | `PH_Mat_Geo_Algo_Init` | 172 | `SUBROUTINE PH_Mat_Geo_Algo_Init(algo)` |
| SUBROUTINE | `PH_Mat_Geo_Algo_Config` | 181 | `SUBROUTINE PH_Mat_Geo_Algo_Config(algo, mapping, max_it, use_num_tang)` |
| SUBROUTINE | `PH_Mat_Geo_Ctx_Init` | 192 | `SUBROUTINE PH_Mat_Geo_Ctx_Init(ctx)` |
| SUBROUTINE | `PH_Mat_Geo_Ctx_Clean` | 199 | `SUBROUTINE PH_Mat_Geo_Ctx_Clean(ctx)` |
