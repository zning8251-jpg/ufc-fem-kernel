...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L5_RT/Material/RT_Mat_Visco_Def.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../05_Project_Planning/PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => RT_Mat_Visco_Desc_Init
    PROCEDURE :: Clean => RT_Mat_Visco_Desc_Clean
  END TYPE
```
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => RT_Mat_Visco_State_Init
    PROCEDURE :: Clean => RT_Mat_Visco_State_Clean
  END TYPE
```
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => RT_Mat_Visco_Algo_Init
  END TYPE
```
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => RT_Mat_Visco_Ctx_Init
    PROCEDURE :: Clean => RT_Mat_Visco_Ctx_Clean
  END TYPE
```
...
|------|------|------|-------------------------|
| SUBROUTINE | `RT_Mat_Visco_Desc_Init` | 74 | `SUBROUTINE RT_Mat_Visco_Desc_Init(this, mat_id, l4_slot)` |
| SUBROUTINE | `RT_Mat_Visco_Desc_Clean` | 80 | `SUBROUTINE RT_Mat_Visco_Desc_Clean(this)` |
| SUBROUTINE | `RT_Mat_Visco_State_Init` | 85 | `SUBROUTINE RT_Mat_Visco_State_Init(this, num_ips)` |
| SUBROUTINE | `RT_Mat_Visco_State_Clean` | 91 | `SUBROUTINE RT_Mat_Visco_State_Clean(this)` |
| SUBROUTINE | `RT_Mat_Visco_Algo_Init` | 96 | `SUBROUTINE RT_Mat_Visco_Algo_Init(this)` |
| SUBROUTINE | `RT_Mat_Visco_Ctx_Init` | 101 | `SUBROUTINE RT_Mat_Visco_Ctx_Init(this)` |
| SUBROUTINE | `RT_Mat_Visco_Ctx_Clean` | 106 | `SUBROUTINE RT_Mat_Visco_Ctx_Clean(this)` |
