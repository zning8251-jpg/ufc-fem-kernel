...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L5_RT/Contact/RT_Cont_Core.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../05_Project_Planning/PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
```fortran
  ...
  CONTAINS
    PROCEDURE, PUBLIC :: Init => RT_Cont_Mgr_Init
    PROCEDURE, PUBLIC :: Clean => RT_Cont_Mgr_Clean
    PROCEDURE, PUBLIC :: Reg => RT_Cont_Mgr_Reg
    PROCEDURE, PUBLIC :: GetStat => RT_Cont_Mgr_GetStat
...
| SUBROUTINE | `RT_Cont_Mgr_Init` | 374 | `SUBROUTINE RT_Cont_Mgr_Init(this, maxContPairs, status)` |
| SUBROUTINE | `RT_Cont_Mgr_Clean` | 402 | `SUBROUTINE RT_Cont_Mgr_Clean(this, status)` |
| SUBROUTINE | `RT_Cont_Mgr_Reg` | 417 | `SUBROUTINE RT_Cont_Mgr_Reg(this, model, status)` |
| SUBROUTINE | `RT_Cont_Mgr_GetStat` | 438 | `SUBROUTINE RT_Cont_Mgr_GetStat(this, nContactPairs)` |
