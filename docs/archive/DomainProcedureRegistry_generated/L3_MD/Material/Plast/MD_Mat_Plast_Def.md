...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L3_MD/Material/Plast/MD_Mat_Plast_Def.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../../05_Project_Planning/PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => MD_Mat_Plast_State_Init
    PROCEDURE :: Update => MD_Mat_Plast_State_Update
    PROCEDURE :: Clean => MD_Mat_Plast_State_Clean
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => MD_Mat_Plast_Algo_Init
    PROCEDURE :: Config => MD_Mat_Plast_Algo_Config
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => MD_Mat_Plast_Ctx_Init
...
|------|------|------|-------------------------|
| SUBROUTINE | `MD_Mat_Plast_Desc_Init` | 244 | `SUBROUTINE MD_Mat_Plast_Desc_Init(desc, sub_type, num_constants, &` |
| SUBROUTINE | `MD_Mat_Plast_Desc_Validate` | 284 | `SUBROUTINE MD_Mat_Plast_Desc_Validate(desc, status)` |
| SUBROUTINE | `MD_Mat_Plast_ComputeDerived` | 337 | `SUBROUTINE MD_Mat_Plast_ComputeDerived(desc, status)` |
| SUBROUTINE | `MD_Mat_Plast_Clean` | 362 | `SUBROUTINE MD_Mat_Plast_Clean(desc, status)` |
| SUBROUTINE | `MD_Mat_Plast_State_Init` | 405 | `SUBROUTINE MD_Mat_Plast_State_Init(state, status)` |
| SUBROUTINE | `MD_Mat_Plast_State_Update` | 431 | `SUBROUTINE MD_Mat_Plast_State_Update(state, stress, strain, eps_pl, &` |
| SUBROUTINE | `MD_Mat_Plast_State_Clean` | 457 | `SUBROUTINE MD_Mat_Plast_State_Clean(state, status)` |
| SUBROUTINE | `MD_Mat_Plast_Algo_Init` | 483 | `SUBROUTINE MD_Mat_Plast_Algo_Init(algo, status)` |
| SUBROUTINE | `MD_Mat_Plast_Algo_Config` | 503 | `SUBROUTINE MD_Mat_Plast_Algo_Config(algo, integration_method, tangent_type, &` |
| SUBROUTINE | `MD_Mat_Plast_Ctx_Init` | 527 | `SUBROUTINE MD_Mat_Plast_Ctx_Init(ctx, status)` |
