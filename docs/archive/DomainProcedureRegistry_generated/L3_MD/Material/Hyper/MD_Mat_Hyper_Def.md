...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L3_MD/Material/Hyper/MD_Mat_Hyper_Def.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../../05_Project_Planning/PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
```fortran
  TYPE, PUBLIC :: MD_Mat_Hyper_State
    REAL(wp) :: J = 1.0_wp
  CONTAINS
    PROCEDURE :: Init => MD_Mat_Hyper_State_Init
    PROCEDURE :: Update => MD_Mat_Hyper_State_Update
    PROCEDURE :: Clean => MD_Mat_Hyper_State_Clean
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => MD_Mat_Hyper_Algo_Init
    PROCEDURE :: Config => MD_Mat_Hyper_Algo_Config
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init => MD_Mat_Hyper_Ctx_Init
...
|------|------|------|-------------------------|
| SUBROUTINE | `MD_Mat_Hyper_Desc_Init` | 129 | `SUBROUTINE MD_Mat_Hyper_Desc_Init(desc, sub_type, num_constants, &` |
| SUBROUTINE | `MD_Mat_Hyper_Desc_Validate` | 166 | `SUBROUTINE MD_Mat_Hyper_Desc_Validate(desc, status)` |
...
| SUBROUTINE | `MD_Mat_Hyper_State_Init` | 245 | `SUBROUTINE MD_Mat_Hyper_State_Init(state, status)` |
| SUBROUTINE | `MD_Mat_Hyper_State_Update` | 260 | `SUBROUTINE MD_Mat_Hyper_State_Update(state, J, status)` |
| SUBROUTINE | `MD_Mat_Hyper_State_Clean` | 276 | `SUBROUTINE MD_Mat_Hyper_State_Clean(state, status)` |
| SUBROUTINE | `MD_Mat_Hyper_Algo_Init` | 291 | `SUBROUTINE MD_Mat_Hyper_Algo_Init(algo, status)` |
| SUBROUTINE | `MD_Mat_Hyper_Algo_Config` | 306 | `SUBROUTINE MD_Mat_Hyper_Algo_Config(algo, formulation, status)` |
| SUBROUTINE | `MD_Mat_Hyper_Ctx_Init` | 322 | `SUBROUTINE MD_Mat_Hyper_Ctx_Init(ctx, status)` |
