...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L3_MD/Material/Elas/MD_Mat_Elas_Def.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../../05_Project_Planning/PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*

## TYPE blocks

### `MD_Mat_Elas_Cfg_Init_Desc` (lines 55–61)
...
### `MD_Mat_Elas_Pop_Vld_Desc` (lines 64–66)
...
### `MD_Mat_Elas_Stp_Evo_Ctx` (lines 69–74)
...
### `MD_Mat_Elas_State` (lines 115–123)
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init   => MD_Mat_Elas_State_Init
    PROCEDURE :: Update => MD_Mat_Elas_State_Update
    PROCEDURE :: Clean  => MD_Mat_Elas_State_Clean
  END TYPE MD_Mat_Elas_State
```

### `MD_Mat_Elas_Algo` (lines 128–136)
...
```fortran
  ...
  CONTAINS
    PROCEDURE :: Init   => MD_Mat_Elas_Algo_Init
    PROCEDURE :: Config => MD_Mat_Elas_Algo_Config
  END TYPE MD_Mat_Elas_Algo
```

### `MD_Mat_Elas_Ctx` (lines 141–146)

```fortran
  TYPE, PUBLIC :: MD_Mat_Elas_Ctx
    TYPE(MD_Mat_Elas_Stp_Evo_Ctx) :: stp  ! Step-level context
  CONTAINS
    PROCEDURE :: Init  => MD_Mat_Elas_Ctx_Init
    PROCEDURE :: Clean => MD_Mat_Elas_Ctx_Clean
  END TYPE MD_Mat_Elas_Ctx
```

### `MD_Mat_Elas_Reg_Arg` (lines 151–163)
...
|------|------|------|-------------------------|
| SUBROUTINE | `MD_Mat_Elas_Desc_Init` | 182 | `SUBROUTINE MD_Mat_Elas_Desc_Init(this, sub_type, num_constants, dependencies, status)` |
| SUBROUTINE | `MD_Mat_Elas_Desc_Valid` | 218 | `SUBROUTINE MD_Mat_Elas_Desc_Valid(this, status)` |
| SUBROUTINE | `MD_Mat_Elas_Desc_ComputeDerived` | 237 | `SUBROUTINE MD_Mat_Elas_Desc_ComputeDerived(this, status)` |
| SUBROUTINE | `MD_Mat_Elas_Desc_Clean` | 274 | `SUBROUTINE MD_Mat_Elas_Desc_Clean(this)` |
| SUBROUTINE | `MD_Mat_Elas_State_Init` | 284 | `SUBROUTINE MD_Mat_Elas_State_Init(this)` |
| SUBROUTINE | `MD_Mat_Elas_State_Update` | 291 | `SUBROUTINE MD_Mat_Elas_State_Update(this, stress, strain)` |
| SUBROUTINE | `MD_Mat_Elas_State_Clean` | 300 | `SUBROUTINE MD_Mat_Elas_State_Clean(this)` |
| SUBROUTINE | `MD_Mat_Elas_Algo_Init` | 311 | `SUBROUTINE MD_Mat_Elas_Algo_Init(this)` |
| SUBROUTINE | `MD_Mat_Elas_Algo_Config` | 318 | `SUBROUTINE MD_Mat_Elas_Algo_Config(this, tangent_type, use_num)` |
| SUBROUTINE | `MD_Mat_Elas_Ctx_Init` | 330 | `SUBROUTINE MD_Mat_Elas_Ctx_Init(this)` |
| SUBROUTINE | `MD_Mat_Elas_Ctx_Clean` | 334 | `SUBROUTINE MD_Mat_Elas_Ctx_Clean(this)` |
| SUBROUTINE | `Build_Aniso_C` | 342 | `SUBROUTINE Build_Aniso_C(constants, C)` |
| FUNCTION | `MD_Mat_Elas_Get_SubType_Name` | 363 | `FUNCTION MD_Mat_Elas_Get_SubType_Name(sub_type) RESULT(name)` |
