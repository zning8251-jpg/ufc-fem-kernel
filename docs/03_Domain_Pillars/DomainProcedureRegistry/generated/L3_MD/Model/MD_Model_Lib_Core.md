...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L3_MD/Model/MD_Model_Lib_Core.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
| SUBROUTINE | `MD_Theory_Describe` | 858 | `SUBROUTINE MD_Theory_Describe(module_id, description, status)` |
| SUBROUTINE | `MD_Theory_Query` | 883 | `SUBROUTINE MD_Theory_Query(module_id, theory_name, layer, status)` |
