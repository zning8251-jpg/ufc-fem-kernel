...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L3_MD/Material/Geo/MD_Mat_Geo_Def.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../../PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
|------|------|------|-------------------------|
| SUBROUTINE | `Desc_Init` | 147 | `SUBROUTINE Desc_Init(desc, sub_type, num_constants, dependencies, status)` |
| SUBROUTINE | `Desc_Validate` | 183 | `SUBROUTINE Desc_Validate(desc, status)` |
| SUBROUTINE | `Desc_ComputeDerived` | 205 | `SUBROUTINE Desc_ComputeDerived(desc, status)` |
| SUBROUTINE | `MD_Mat_Geo_Clean` | 224 | `SUBROUTINE MD_Mat_Geo_Clean(desc, status)` |
| SUBROUTINE | `State_Init` | 253 | `SUBROUTINE State_Init(state, status)` |
| SUBROUTINE | `State_Update` | 275 | `SUBROUTINE State_Update(state, stress, strain, eps_pl, &` |
| SUBROUTINE | `State_Clean` | 301 | `SUBROUTINE State_Clean(state, status)` |
| SUBROUTINE | `Algo_Init` | 323 | `SUBROUTINE Algo_Init(algo, status)` |
| SUBROUTINE | `Algo_Config` | 342 | `SUBROUTINE Algo_Config(algo, return_mapping, max_iter, &` |
| SUBROUTINE | `Ctx_Init` | 364 | `SUBROUTINE Ctx_Init(ctx, status)` |
