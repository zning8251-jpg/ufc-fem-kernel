...
- **Registry 布局（镜像 `ufc_core` 相对路径 + `.md`）**: `generated/L4_PH/Material/Creep/PH_Mat_Creep_Def.md` — *与 [`UFC_ufc_core_目录权威分类.md`](../../../../../PPLAN/06_核心架构/UFC_ufc_core_目录权威分类.md) 物理树一致；三段式/四段式解析见上*
...
|------|------|------|-------------------------|
| SUBROUTINE | `Desc_Init` | 102 | `SUBROUTINE Desc_Init(desc, sub_type, nprops)` |
| SUBROUTINE | `Desc_Valid` | 112 | `SUBROUTINE Desc_Valid(desc, status)` |
| SUBROUTINE | `Desc_Clean` | 122 | `SUBROUTINE Desc_Clean(desc)` |
| SUBROUTINE | `State_Init` | 133 | `SUBROUTINE State_Init(state)` |
| SUBROUTINE | `State_Update` | 143 | `SUBROUTINE State_Update(state, stress, creep_strain)` |
| SUBROUTINE | `State_Clean` | 151 | `SUBROUTINE State_Clean(state)` |
| SUBROUTINE | `Algo_Init` | 163 | `SUBROUTINE Algo_Init(algo)` |
| SUBROUTINE | `Algo_Config` | 169 | `SUBROUTINE Algo_Config(algo, method, use_num_tang)` |
| SUBROUTINE | `Ctx_Init` | 179 | `SUBROUTINE Ctx_Init(ctx)` |
| SUBROUTINE | `Ctx_Clean` | 186 | `SUBROUTINE Ctx_Clean(ctx)` |
