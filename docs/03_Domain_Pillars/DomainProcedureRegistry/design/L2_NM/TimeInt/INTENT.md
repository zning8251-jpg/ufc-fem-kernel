# `L2_NM` / `TimeInt` 设计意图（域桶）

> **域桶**：`ufc_core/L2_NM/TimeInt/`（含子域目录）。  
> **验收向**：本 INTENT + [`CONVENTIONS.md`](../../../CONVENTIONS.md) + 域内 `CONTRACT.md`（若存在）+ `manifest.json` ↔ `align.py`。

## 1. 问题与目标

- **职责**：本目录为 **六层架构** 中 `L2_NM` 之下的一级 **域桶**；子目录为子域，**`manifest.json`** 仍按 **整桶递归** 与 `ufc_core` / `generated/` 对账。
...
- 域合同：[`CONTRACT.md`](../../../../../ufc_core/L2_NM/TimeInt/CONTRACT.md)
