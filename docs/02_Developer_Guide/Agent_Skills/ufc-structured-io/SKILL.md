...
1. **规范主文档**：
   [`docs/05_Project_Planning/PPLAN/04_技术标准/UFC_Principle14_结构化IO参数传递规范.md`](../../../05_Project_Planning/PPLAN/04_技术标准/UFC_Principle14_结构化IO参数传递规范.md)
...
2. **已完成基准实现**（对照参考）：
   - Assembly 域：[`UFC/ufc_core/L5_RT/Assembly/RT_Asm_Proc.f90`](../../../ufc_core/L5_RT/Assembly/RT_Asm_Proc.f90)
   - LoadBC 域：[`UFC/ufc_core/L5_RT/LoadBC/RT_LoadBC_Proc.f90`](../../../ufc_core/L5_RT/LoadBC/RT_LoadBC_Proc.f90)
   - Solver 域（v2.0）：[`UFC/ufc_core/L5_RT/Solver/RT_Solv_Proc.f90`](../../../ufc_core/L5_RT/Solver/RT_Solv_Proc.f90)
   - Material 域（v2.0）：[`UFC/ufc_core/L5_RT/Material/RT_Mat_Proc.f90`](../../../ufc_core/L5_RT/Material/RT_Mat_Proc.f90)

3. **域专属 Types 文件**（生成 _Proc 前确认四大类 TYPE 名）：
   目标域的 `RT_XXX_Types.f90`（同目录）

4. **统一 IO 模板**（与规范一致的首选骨架）：
   [`UFC/docs/templates/`](../../templates/) 下 `RT_XXX_Load_Proc.f90`、`RT_XXX_Mat_Proc.f90`、`RT_XXX_Output_Proc.f90`、`RT_XXX_StepDriver_Proc.f90` 等 — **`*_Arg` + `args` 哑元**，非 `inp`/`out`。
