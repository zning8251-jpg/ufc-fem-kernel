...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE IF_Registry_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[IF_Registry_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE IF_Registry_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call IF_Registry_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call IF_Registry_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
