...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE NM_Solver_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[NM_Solver_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE NM_Solver_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call NM_Solver_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call NM_Solver_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
