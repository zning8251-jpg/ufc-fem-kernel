...
! Layer:   L2_NM
! Domain:  Base
! Purpose: Minimal test framework for the Base domain.
...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE NM_Base_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[NM_Base_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE NM_Base_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call NM_Base_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call NM_Base_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
