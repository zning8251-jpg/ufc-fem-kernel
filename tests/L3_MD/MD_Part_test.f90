...
! Layer:   L3_MD
! Domain:  Part
! Purpose: Minimal test framework for the Part domain.
...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE MD_Part_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[MD_Part_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE MD_Part_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call MD_Part_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call MD_Part_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
