...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE MD_Output_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[MD_Output_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE MD_Output_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call MD_Output_Domain % Init / Finalize (MD_Out_API) with minimal valid inputs
