...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE PH_Mat_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[PH_Mat_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE PH_Mat_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call PH_Mat_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call PH_Mat_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
