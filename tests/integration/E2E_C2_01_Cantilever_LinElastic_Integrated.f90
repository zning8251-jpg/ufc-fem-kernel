!===============================================================================
! E2E Test: C2-01 Cantilever Beam Linear Elastic Static (INTEGRATED VERSION)
! Layer:  Integration (L3->L4->L5 cross-layer)
! Domain: Element + Material + Load + BC + Assembly + Solver
!
! Integration Status: PARTIAL
!   - USE PH_Elem_Solid3D_EAS: Types (PH_EAS_Ctx, PH_EAS_NMODES) integrated
!   - USE RT_Asm_Def: Assembly type definitions (RT_Asm_Desc) integrated
!   - USE RT_Asm_Core: Scatter/BC operations (interface mismatch → inline retained)
!   - Inline retained: C3D8+IM element stiffness (module uses EAS 9-param, not IM)
!   - Inline retained: Gauss solver, BC penalty (module expects pointer-based state)
!
! Modules Available (.mod): PH_Elem_Solid3D_EAS, RT_Asm_Def, RT_Asm_Core,
!                           IF_Prec_Core, IF_Err_Brg
! Modules Missing (.mod): None needed beyond above
!
! Original: E2E_C2_01_Cantilever_LinElastic.f90 (self-contained, PASS)
! Status: ACTIVE | Created: 2026-04-28 | Integrated: 2026-04-28
!===============================================================================
program E2E_C2_01_Cantilever_LinElastic_Integrated
  ! === INTEGRATED: USE actual UFC modules for precision and types ===
  ! TODO: Integrate with PH_Elem_Solid3D_EAS when EAS kernel supports
  !       direct Ke computation with user-supplied D matrix and coords.
  !       Current EAS interface (PH_EAS_ComputeKe) requires PH_EAS_Ctx
  !       with full GP infrastructure, not compatible with standalone test.
  ! TODO: Integrate with RT_Asm_Core when interface stabilizes for dense
  !       matrix assembly (current RT_Asm_Core_Scatter_Ke expects RT_Asm_State
  !       with pointer-based K_global, not stack-allocated arrays).
  implicit none
  ...
  integer(i4), parameter :: n_node = nnx*nny*nnz
  integer(i4), parameter :: n_dof  = n_node * 3
  ...
  print '(A)', '=== E2E Test C2-01: Cantilever_LinElastic [INTEGRATED] ==='
  ...
  ! === L3: Material (inline — no MD_Mat_Def .mod available) ===
  print '(A)', '[L3] Building isotropic elastic D matrix ...'
  call build_elastic_D(E_mod, nu_mat, D_mat)

  ! === L4+L5: Element stiffness + assembly ===
  ! TODO: Integrate with PH_Elem_Solid3D_EAS when interface stabilizes
  ! Current inline: C3D8+Incompatible Modes (Wilson-Taylor, 9 internal DOFs)
...
contains
  ...
  ! --- Inline retained: build_elastic_D ---
  ! TODO: Integrate with actual module when interface stabilizes
  ...
  ! --- Inline retained: C3D8+IM element stiffness ---
  ! TODO: Integrate with PH_Elem_Solid3D_EAS when EAS kernel supports
  !       direct Ke computation with user D and coords (current interface
  !       requires PH_EAS_Ctx with full GP infrastructure)
  subroutine compute_C3D8_IM(all_coords, ec, D, Ke)
    ...
    do ig = 1, 2; do jg = 1, 2; do kg = 1, 2
      ...
      call invert3x3(J, Jinv, detJ); w = abs(detJ)
      dN_dx = 0.0_wp
      do a = 1, 8; do ii = 1, 3; do jj = 1, 3
      ...
      do a = 1, 8
        kk = (a-1)*3
        B(1,kk+1)=dN_dx(a,1); B(2,kk+2)=dN_dx(a,2); B(3,kk+3)=dN_dx(a,3)
      ...
      G = 0.0_wp
      do idir = 1, 3; do m = 1, 3
        q = (idir-1)*3 + m
        dMm_dx(1) = -2.0_wp*xi_nat(m)*J0inv(m,1)
        dMm_dx(2) = -2.0_wp*xi_nat(m)*J0inv(m,2)
        dMm_dx(3) = -2.0_wp*xi_nat(m)*J0inv(m,3)
        select case(idir)
        case(1); G(1,q)=dMm_dx(1); G(4,q)=dMm_dx(2); G(6,q)=dMm_dx(3)
        case(2); G(2,q)=dMm_dx(2); G(4,q)=dMm_dx(1); G(5,q)=dMm_dx(3)
        case(3); G(3,q)=dMm_dx(3); G(5,q)=dMm_dx(2); G(6,q)=dMm_dx(1)
        end select
      end do; end do
      DB = 0.0_wp; DG = 0.0_wp
      do ii = 1, 6
        do jj = 1, 24; do kk = 1, 6; DB(ii,jj)=DB(ii,jj)+D(ii,kk)*B(kk,jj); end do; end do
        do jj = 1, 9;  do kk = 1, 6; DG(ii,jj)=DG(ii,jj)+D(ii,kk)*G(kk,jj); end do; end do
      end do
      do ii=1,24; do jj=1,24; do kk=1,6; Kuu(ii,jj)=Kuu(ii,jj)+B(kk,ii)*DB(kk,jj)*w; end do; end do; end do
      do ii=1,24; do jj=1,9;  do kk=1,6; Kua(ii,jj)=Kua(ii,jj)+B(kk,ii)*DG(kk,jj)*w; end do; end do; end do
      do ii=1,9;  do jj=1,9;  do kk=1,6; Kaa(ii,jj)=Kaa(ii,jj)+G(kk,ii)*DG(kk,jj)*w; end do; end do; end do
    end do; end do; end do
    call invert_small(Kaa, Kaa_inv, 9)
    temp = 0.0_wp
    do ii=1,24; do jj=1,9; do kk=1,9; temp(ii,jj)=temp(ii,jj)+Kua(ii,kk)*Kaa_inv(kk,jj); end do; end do; end do
    Ke = Kuu
    do ii=1,24; do jj=1,24; do kk=1,9; Ke(ii,jj)=Ke(ii,jj)-temp(ii,kk)*Kua(jj,kk); end do; end do; end do
  ...
  subroutine invert_small(A, Ainv, n)
    ...
    W = 0.0_wp
    do ii = 1, n; do jj = 1, n; W(ii,jj) = A(ii,jj); end do; end do
    do ii = 1, n; W(ii,n+ii) = 1.0_wp; end do
    do kk = 1, n
      pr = kk
      do ii = kk+1, n; if (abs(W(ii,kk)) > abs(W(pr,kk))) pr = ii; end do
      if (pr /= kk) then
        do jj = 1, 2*n; tmp = W(kk,jj); W(kk,jj) = W(pr,jj); W(pr,jj) = tmp; end do
      end if
      piv = W(kk,kk)
      do jj = 1, 2*n; W(kk,jj) = W(kk,jj) / piv; end do
      do ii = 1, n
        if (ii /= kk) then
          piv = W(ii,kk)
          do jj = 1, 2*n; W(ii,jj) = W(ii,jj) - piv*W(kk,jj); end do
        end if
      end do
    end do
    do ii = 1, n; do jj = 1, n; Ainv(ii,jj) = W(ii,n+jj); end do; end do
  end subroutine

  ! --- Inline retained: scatter, BC, solver ---
  ! TODO: Integrate with RT_Asm_Core when interface stabilizes
  subroutine scatter_Ke(Ke, ec, K_g, ndg)
    ...
    do a=1,8; do b=1,8; do li=1,3; do lj=1,3
  ...
  subroutine apply_bc_penalty(K, F, bc, nb, nd)
    ...
    integer(i4) :: ii
    do ii = 1, nb; K(bc(ii),bc(ii))=K(bc(ii),bc(ii))+pen; F(bc(ii))=0.0_wp; end do
  end subroutine
  ...
  subroutine solve_gauss(A, b, x, n)
    ...
    do kk = 1, n-1
      ...
      do ii=kk+1,n; if(abs(A(ii,kk))>mx) then; mx=abs(A(ii,kk)); pr=ii; end if; end do
      if (pr/=kk) then
        do jj=1,n; tmp=A(kk,jj); A(kk,jj)=A(pr,jj); A(pr,jj)=tmp; end do
        tmp=b(kk); b(kk)=b(pr); b(pr)=tmp
      end if
      do ii=kk+1,n
        if(abs(A(kk,kk))>1e-30_wp) then
          fac=A(ii,kk)/A(kk,kk)
          do jj=kk+1,n; A(ii,jj)=A(ii,jj)-fac*A(kk,jj); end do
          b(ii)=b(ii)-fac*b(kk)
        end if
      end do
    end do
    ...
    do ii=n,1,-1
      x(ii) = b(ii)
      do jj=ii+1,n; x(ii)=x(ii)-A(ii,jj)*x(jj); end do
      if(abs(A(ii,ii))>1e-30_wp) x(ii)=x(ii)/A(ii,ii)
    end do
  end subroutine

end program E2E_C2_01_Cantilever_LinElastic_Integrated
