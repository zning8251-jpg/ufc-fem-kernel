...
! Layer:   L6_AP
! Domain:  UI
! Purpose: Minimal test framework for the UI domain.
...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE AP_UI_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[AP_UI_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE AP_UI_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call AP_UI_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call AP_UI_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
