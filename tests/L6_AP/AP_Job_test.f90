...
! Layer:   L6_AP
! Domain:  Job
! Purpose: Minimal test framework for the Job domain.
...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE AP_Job_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[AP_Job_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE AP_Job_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call AP_Job_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call AP_Job_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
