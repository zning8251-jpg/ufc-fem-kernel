...
CONTAINS
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE RT_Solver_Run_Tests(all_passed)
    ...
    WRITE(*,'(A,I4,A,I4,A)') "[RT_Solver_Test] ", n_passed, " passed, ", &
                               n_failed, " failed"
  END SUBROUTINE RT_Solver_Run_Tests
  ...
  SUBROUTINE test_init_finalize()
    ...
    ! TODO: Call RT_Solv_Core Init with minimal valid inputs
    ! TODO: Verify status == IF_STATUS_OK
    ! TODO: Call RT_Solv_Core Finalize
    ! TODO: Verify status == IF_STATUS_OK
