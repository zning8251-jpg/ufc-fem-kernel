...
### 3.2 SIO：`Elem_Ke_In` ↔ `PH_Element_Compute_Ke_Arg`（G5，change `p2-element-l5-rt-elem-proc-sio`）

| `PH_Element_Compute_Ke_Arg` | `Elem_Ke_In` (`RT_Elem_Proc`) | 填充 |
|-----------------------------|-------------------------------|------|
| `elem_idx` | `elem_idx` | `RT_Elem_KeIn_ApplyAsmArg` |
| `l3_elem_idx` | `l3_elem_idx` | 同上 |
| `mat_pt_idx` | `mat_pt_idx` | 同上 |
| `nDof` | `nDof` | 同上 |
| — | `coords` | 指针 → 调用方 `TARGET` 坐标缓存 |
| — | `u` | 可选单元位移 |
| `mat_props_in` | `mat_props_in` | 自 `RT_Asm_Solv_KeArg_AttachMatProps` 拷贝 |
| `evo%Ke` | — | **仅金线**；SIO 输出在 `Elem_Ke_Out%Ke` |

**生产路径不变**：`RT_Asm_Solv` → `PH_Element_Compute_Ke_Arg` → `PH_Elem_Domain%Compute_Ke`。  
**试验路径**：`RT_Elem_Brg_ComputeKe` → `PH_ElemKeDispatch%Compute_Ke`（须先 `AttachMatProps` + `RT_Elem_KeIn_ApplyAsmArg`）。  
**静态门控**：`tools/verify_rt_elem_ke_in_align.py`。
...
| `RT_Elem_KeIn_ApplyAsmArg` | `RT_ElemDispatch_Brg` | SIO：从 `PH_Element_Compute_Ke_Arg` 填充 `Elem_Ke_In` |
