...
! array iw will be used to test for duplicates
 ...
 subroutine generate_lists(point_type, info, lists, lists_ends, &
  ...
  do point = 1,n_points
   ...
   if (point_type(point) .ne. -2) then
    ...
    lists(point)%itr%weight = point_type(point)
   ...
   if (lists(point)%itr%weight>0) then
    ...
    ! update max_weight
    if (lists(point)%itr%weight > max_weight) then
     max_weight = lists(point)%itr%weight
    end if
 ...
 subroutine list_promote_point(point, weight_inc, lists, lists_ends, info)
  ...
  lists(point)%itr%weight = weight + weight_inc
    ...
    do index_i = str_t%ptr(new_C), str_t%ptr(new_C+1)-1
     ...
     do index_k = row_i_start+1, row_i_end ! bypass diagonal
      ...
      if ( (point_type(point_k) == 0) .and. &
       ...
       if ( lists(point_k)%itr%weight > max_weight ) then
        max_weight = lists(point_k)%itr%weight
       end if
      end if
     end do
    end do
