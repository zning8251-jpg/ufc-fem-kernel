!===============================================================================
! MODULE: MD_Mat_User_Def
! LAYER:  L3_MD
! DOMAIN: Material / User
! ROLE:   Def
! BRIEF:  Unified TYPE definitions for user material family.
...
MODULE MD_Mat_User_Def
  ...
  ! Owner: L3_MD layer
  !=======================================================================
  TYPE, PUBLIC, EXTENDS(MD_Mat_Desc) :: MD_Mat_User_Desc
    !--- Auxiliary nesting ---
    TYPE(MD_Mat_User_Cfg_Init_Desc) :: cfg
    TYPE(MD_Mat_User_Pop_Vld_Desc)  :: pop
...
CONTAINS
  ...
  !=============================================================================
  ! TBP IMPLEMENTATIONS: MD_Mat_User_Desc
  !=============================================================================

  SUBROUTINE Desc_Init(this, sub_type, num_constants, dependencies, &
                                   num_state_vars, status)
    CLASS(MD_Mat_User_Desc), INTENT(INOUT) :: this
    INTEGER(i4),             INTENT(IN)    :: sub_type
    INTEGER(i4),             INTENT(IN)    :: num_constants
    INTEGER(i4),             INTENT(IN), OPTIONAL :: dependencies
    INTEGER(i4),             INTENT(IN), OPTIONAL :: num_state_vars
    TYPE(ErrorStatusType),   INTENT(OUT)   :: status

    CALL init_error_status(status)

    this%cfg%family_type    = MD_MAT_FAMILY_USER
    ...
    IF (PRESENT(num_state_vars)) THEN
      this%cfg%num_state_vars = num_state_vars
    END IF
  ...
  SUBROUTINE Desc_Valid(this, status)
    CLASS(MD_Mat_User_Desc), INTENT(IN)  :: this
    TYPE(ErrorStatusType),   INTENT(OUT) :: status
    ...
    END IF
    IF (this%cfg%family_type /= MD_MAT_FAMILY_USER) THEN
      status%status_code = -2; status%message = "Not USER family"; RETURN
    END IF
    IF (this%cfg%sub_type < 1101 .OR. this%cfg%sub_type > 1102) THEN
  ...
  SUBROUTINE Desc_Clean(this)
    ...
    IF (ALLOCATED(this%constants)) DEALLOCATE(this%constants)
    this%density            = 0.0_wp
    this%cfg%num_state_vars = 0
    this%pop%is_initialized = .FALSE.
  END SUBROUTINE Desc_Clean

  !=============================================================================
  ! TBP IMPLEMENTATIONS: MD_Mat_User_State
  !=============================================================================

  SUBROUTINE State_Init(this, n_state_vars)
    CLASS(MD_Mat_User_State), INTENT(OUT) :: this
    INTEGER(i4), INTENT(IN), OPTIONAL :: n_state_vars

    IF (ALLOCATED(this%statev)) DEALLOCATE(this%statev)
    IF (PRESENT(n_state_vars) .AND. n_state_vars > 0) THEN
      ALLOCATE(this%statev(n_state_vars))
      this%statev = 0.0_wp
    END IF
  END SUBROUTINE State_Init

  SUBROUTINE State_Update(this, statev)
    CLASS(MD_Mat_User_State), INTENT(INOUT) :: this
    REAL(wp),                 INTENT(IN)    :: statev(:)

    IF (.NOT. ALLOCATED(this%statev)) THEN
      ALLOCATE(this%statev(SIZE(statev)))
    END IF
    this%statev = statev
  ...
  !=============================================================================
  ! TBP IMPLEMENTATIONS: MD_Mat_User_Algo
  !=============================================================================

  SUBROUTINE Algo_Init(this)
    CLASS(MD_Mat_User_Algo), INTENT(OUT) :: this
    this%integration_method = 1_i4
  END SUBROUTINE Algo_Init

  !=============================================================================
  ! TBP IMPLEMENTATIONS: MD_Mat_User_Ctx
  !=============================================================================

  SUBROUTINE Ctx_Init(this)
    CLASS(MD_Mat_User_Ctx), INTENT(OUT) :: this
  END SUBROUTINE Ctx_Init

  SUBROUTINE Ctx_Clean(this)
    CLASS(MD_Mat_User_Ctx), INTENT(INOUT) :: this
