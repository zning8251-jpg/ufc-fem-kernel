...
! ROLE:   Proc
! BRIEF:  C3D5 element definition (5-node pyramid)
!===============================================================================
MODULE PH_Elem_C3D5
...
!> Merged: Defn + Sect + Constraints + Cont + Loads + Out
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: PH_Elem_C3D5_DefInit
  PUBLIC :: PH_Elem_C3D5_ShapeFunc
  PUBLIC :: PH_Elem_C3D5_Jac
  PUBLIC :: PH_Elem_C3D5_BMatrix
  PUBLIC :: PH_Elem_C3D5_GaussPoints
  PUBLIC :: PH_Elem_C3D5_JacB
  PUBLIC :: PH_Elem_C3D5_Strain
  PUBLIC :: PH_Elem_C3D5_Stress
  PUBLIC :: PH_Elem_C3D5_ConstMatrix
  PUBLIC :: PH_Elem_C3D5_FormStiffMatrix
  PUBLIC :: PH_Elem_C3D5_FormStiffMatrixFromD
  PUBLIC :: PH_Elem_C3D5_FormIntForce
  PUBLIC :: PH_Elem_C3D5_FormIntForceFromStress
  PUBLIC :: PH_Elem_C3D5_ConsMass
  PUBLIC :: PH_Elem_C3D5_LumpMass
  PUBLIC :: PH_Elem_C3D5_ThermStrainVector
  PUBLIC :: PH_ELEM_C3D5_NNODE
  PUBLIC :: PH_ELEM_C3D5_NIP
  PUBLIC :: PH_ELEM_C3D5_NDOF
  PUBLIC :: PH_ELEM_C3D5_NFACE
  PUBLIC :: PH_ELEM_C3D5_FACE_NODES
  PUBLIC :: PH_Elem_C3D5_NL_TL
  PUBLIC :: PH_Elem_C3D5_NL_UL
  PUBLIC :: PH_Elem_C3D5_Material_Update_Routed
  PUBLIC :: PH_Elem_C3D5_GetVolume, PH_Elem_C3D5_GetSectProps, PH_Elem_C3D5_GetCentroid
  PUBLIC :: PH_Elem_C3D5_GetInertiaOrig
  PUBLIC :: PH_Elem_C3D5_ApplyConstraint, PH_Elem_C3D5_ApplyMPC
  PUBLIC :: PH_Elem_C3D5_FormContactContrib, PH_Elem_C3D5_FormContactFaceCtr
  PUBLIC :: PH_Elem_C3D5_FormNodalForce, PH_Elem_C3D5_FormBodyForce, PH_Elem_C3D5_FormFacePressure
  PUBLIC :: PH_Elem_C3D5_CollectIPVars, PH_Elem_C3D5_MapToNode, PH_Elem_C3D5_GetExtrapMat
  PUBLIC :: PH_Elem_C3D5_EvalVonMises, PH_Elem_C3D5_EvalPrincStress
  PUBLIC :: PH_Elem_C3D5_EvalStressInvar, PH_Elem_C3D5_EvalStrainInvar, PH_Elem_C3D5_EvalTriaxiality
  PUBLIC :: PH_ELEM_CTYPE_PENALTY_DOF, PH_ELEM_CTYPE_MPC_LINEAR
  PUBLIC :: PH_ELEM_LOAD_BODY, PH_ELEM_LOAD_FACE_P

  INTEGER(i4), PARAMETER :: PH_ELEM_C3D5_NNODE  = 5_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D5_NIP   = 8_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D5_NDOF  = 15_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D5_NFACE = 5_i4
  REAL(wp), PARAMETER :: PH_ELEM_GAUSS_PT = 0.577350269189626_wp
  ! Face: 1=base quad(1,2,3,4); 2?=triangles (1,2,5),(2,3,5),(3,4,5),(4,1,5). Tri: 4th=0.
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D5_FACE_NODES(4, 5) = RESHAPE([ &
    1,2,3,4, 1,2,5,0, 2,3,5,0, 3,4,5,0, 4,1,5,0 ], [4, 5])
...
CONTAINS
  ...
  SUBROUTINE PH_Elem_C3D5_ThermStrainVector(alpha, deltaT, eps_th)
    REAL(wp), INTENT(IN)  :: alpha, deltaT
    REAL(wp), INTENT(OUT) :: eps_th(6)
    eps_th(1:3) = alpha * deltaT
    eps_th(4:6) = ZERO
  END SUBROUTINE PH_Elem_C3D5_ThermStrainVector

  SUBROUTINE PH_Elem_C3D5_BMatrix(dNdx, B)
    REAL(wp), INTENT(IN)  :: dNdx(3, 5)
    REAL(wp), INTENT(OUT) :: B(6, 15)
    INTEGER(i4) :: i, c
    B = ZERO
    DO i = 1, 5
      c = 3 * (i - 1) + 1
      ...
      B(6, c+2) = dNdx(1, i)
    END DO
  END SUBROUTINE PH_Elem_C3D5_BMatrix

  SUBROUTINE PH_Elem_C3D5_ConsMass(coords, rho, Me)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: rho
    REAL(wp), INTENT(OUT) :: Me(15, 15)
    REAL(wp) :: N(5), dNdxi(3, 5), J(3, 3), detJ
    REAL(wp) :: xi(8), eta(8), zeta(8), weights(8)
    REAL(wp) :: dV, nij
    INTEGER(i4) :: ip, i, j
    Me = ZERO
    CALL PH_Elem_C3D5_GaussPoints(xi, eta, zeta, weights)
    DO ip = 1, 8
      CALL PH_Elem_C3D5_ShapeFunc(xi(ip), eta(ip), zeta(ip), N, dNdxi)
      CALL PH_Elem_C3D5_Jac(dNdxi, coords, J, detJ)
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D5_ConsMass

  SUBROUTINE PH_Elem_C3D5_ConstMatrix(E_young, nu, D)
    ...
    D(5,5) = D(4,4)
    D(6,6) = D(4,4)
  END SUBROUTINE PH_Elem_C3D5_ConstMatrix

  SUBROUTINE PH_Elem_C3D5_DefInit()
  END SUBROUTINE PH_Elem_C3D5_DefInit

  SUBROUTINE PH_Elem_C3D5_FormIntForce(coords, u, E_young, nu, R_int)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: u(15)
    REAL(wp), INTENT(IN)  :: E_young, nu
    REAL(wp), INTENT(OUT) :: R_int(15)
    REAL(wp) :: Ke(15, 15)
    CALL PH_Elem_C3D5_FormStiffMatrix(coords, E_young, nu, Ke)
    R_int = MATMUL(Ke, u)
  END SUBROUTINE PH_Elem_C3D5_FormIntForce

  SUBROUTINE PH_Elem_C3D5_FormIntForceFromStress(coords, sigma6, R_int)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: sigma6(6)
    REAL(wp), INTENT(OUT) :: R_int(15)
    REAL(wp) :: N(5), dNdx(3, 5), J(3, 3), detJ, B(6, 15)
    REAL(wp) :: xi(8), eta(8), zeta(8), weights(8)
    REAL(wp) :: dV
    INTEGER(i4) :: ip
    R_int = ZERO
    CALL PH_Elem_C3D5_GaussPoints(xi, eta, zeta, weights)
    DO ip = 1, 8
      CALL PH_Elem_C3D5_JacB(coords, xi(ip), eta(ip), zeta(ip), N, dNdx, J, detJ, B)
      IF (ABS(detJ) <= 1.0e-12_wp) CYCLE
      dV = detJ * weights(ip)
      R_int = R_int + dV * MATMUL(TRANSPOSE(B), sigma6)
    END DO
  END SUBROUTINE PH_Elem_C3D5_FormIntForceFromStress

  SUBROUTINE PH_Elem_C3D5_FormStiffMatrix(coords, E_young, nu, Ke)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: E_young, nu
    REAL(wp), INTENT(OUT) :: Ke(15, 15)
    REAL(wp) :: N(5), dNdx(3, 5), J(3, 3), detJ, B(6, 15), D(6, 6)
    REAL(wp) :: xi(8), eta(8), zeta(8), weights(8)
    INTEGER(i4) :: ip
    Ke = ZERO
    CALL PH_Elem_C3D5_ConstMatrix(E_young, nu, D)
    CALL PH_Elem_C3D5_GaussPoints(xi, eta, zeta, weights)
    DO ip = 1, 8
      CALL PH_Elem_C3D5_JacB(coords, xi(ip), eta(ip), zeta(ip), N, dNdx, J, detJ, B)
      IF (ABS(detJ) <= 1.0e-12_wp) CYCLE
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D5_FormStiffMatrix

  SUBROUTINE PH_Elem_C3D5_FormStiffMatrixFromD(coords, D6, Ke)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D5_FormStiffMatrixFromD

  SUBROUTINE PH_Elem_C3D5_GaussPoints(xi, eta, zeta, weights)
    ...
    DO iy = 1, 2
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D5_GaussPoints

  SUBROUTINE PH_Elem_C3D5_GetVolumeInt(coords, volume)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D5_GetVolumeInt

  SUBROUTINE PH_Elem_C3D5_Jac(dNdxi, coords, J, detJ)
    REAL(wp), INTENT(IN)  :: dNdxi(3, 5)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
  ...
  END SUBROUTINE PH_Elem_C3D5_Jac

  SUBROUTINE PH_Elem_C3D5_JacB(coords, xi_pt, eta_pt, zeta_pt, N, dNdx, J, detJ, B)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: xi_pt, eta_pt, zeta_pt
    REAL(wp), INTENT(OUT) :: N(5)
    REAL(wp), INTENT(OUT) :: dNdx(3, 5)
    REAL(wp), INTENT(OUT) :: J(3, 3)
    REAL(wp), INTENT(OUT) :: detJ
    REAL(wp), INTENT(OUT) :: B(6, 15)
    REAL(wp) :: dNdxi(3, 5), J_inv(3, 3)
    INTEGER(i4) :: i, j, k
    CALL PH_Elem_C3D5_ShapeFunc(xi_pt, eta_pt, zeta_pt, N, dNdxi)
    CALL PH_Elem_C3D5_Jac(dNdxi, coords, J, detJ)
    ...
    DO i = 1, 5
      ...
      END DO
    END DO
    CALL PH_Elem_C3D5_BMatrix(dNdx, B)
  END SUBROUTINE PH_Elem_C3D5_JacB

  SUBROUTINE PH_Elem_C3D5_LumpMass(coords, rho, M_lumped)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: rho
    REAL(wp), INTENT(OUT) :: M_lumped(15)
    REAL(wp) :: V, m
    INTEGER(i4) :: i
    CALL PH_Elem_C3D5_GetVolumeInt(coords, V)
    m = rho * V / 5.0_wp
    DO i = 1, 5
      M_lumped(3*i-2) = m
      M_lumped(3*i-1) = m
      M_lumped(3*i)   = m
    END DO
  END SUBROUTINE PH_Elem_C3D5_LumpMass

  SUBROUTINE PH_Elem_C3D5_NL_TL(coords_ref, u_elem, mat_prop, mat_state, Ke_mat, Ke_geo, R_int, status)
    REAL(wp), INTENT(IN) :: coords_ref(3, 5)
    REAL(wp), INTENT(IN) :: u_elem(15)
    ...
    REAL(wp) :: coords_curr(3, 5)
    REAL(wp) :: xi_gp(8), eta_gp(8), zeta_gp(8), wt_gp(8)
    REAL(wp) :: N(5), dN_dxi(3, 5), dN_dX(3, 5)
    ...
    REAL(wp) :: K_mat_gp(15, 15), K_geo_gp(15, 15), R_gp(15)
    TYPE(RT_LagrCfg) :: cfg
    INTEGER(i4) :: i, igp, j
    ...
    CALL PH_Elem_C3D5_GaussPoints(xi_gp, eta_gp, zeta_gp, wt_gp)
    ...
    DO igp = 1, PH_ELEM_C3D5_NIP
      CALL PH_Elem_C3D5_ShapeFunc(xi_gp(igp), eta_gp(igp), zeta_gp(igp), N, dN_dxi)
      CALL PH_Elem_C3D5_Jac(dN_dxi, coords_ref, J_ref, det_J)
  ...
  END SUBROUTINE PH_Elem_C3D5_NL_TL

  SUBROUTINE PH_Elem_C3D5_NL_UL(coords_prev, u_incr, mat_prop, mat_state, Ke_mat, Ke_geo, R_int, status)
    REAL(wp), INTENT(IN) :: coords_prev(3, 5)
    REAL(wp), INTENT(IN) :: u_incr(15)
    TYPE(MatPropertyDef), INTENT(IN) :: mat_prop
    TYPE(PH_MatPoint_State), INTENT(INOUT) :: mat_state(:)
    REAL(wp), INTENT(OUT) :: Ke_mat(15, 15)
    REAL(wp), INTENT(OUT) :: Ke_geo(15, 15)
    ...
    DO igp = 1, PH_ELEM_C3D5_NIP
      CALL PH_Elem_C3D5_ShapeFunc(xi_gp(igp), eta_gp(igp), zeta_gp(igp), N, dN_dxi)
      CALL PH_Elem_C3D5_Jac(dN_dxi, coords_prev, J_prev, det_J)
  ...
  CONTAINS
    SUBROUTINE Invert3x3(A, A_inv, det)
      ...
      A_inv(3,3) = (A(1,1)*A(2,2) - A(1,2)*A(2,1)) / det
    END SUBROUTINE Invert3x3
  END SUBROUTINE PH_Elem_C3D5_NL_UL

  SUBROUTINE PH_Elem_C3D5_ShapeFunc(xi, eta, zeta, N, dNdxi)
    REAL(wp), INTENT(IN)  :: xi, eta, zeta
    REAL(wp), INTENT(OUT) :: N(5)
    REAL(wp), INTENT(OUT) :: dNdxi(3, 5)
    REAL(wp) :: cz
    ! Master: xi,eta in [-1,1]; zeta in [0,1] (base zeta=0, apex zeta=1).
    ! N1..N4: bilinear on base scaled by (1-zeta); N5=zeta (apex).
    cz = ONE - zeta
    N(1) = 0.25_wp * (ONE - xi) * (ONE - eta) * cz
    N(2) = 0.25_wp * (ONE + xi) * (ONE - eta) * cz
    N(3) = 0.25_wp * (ONE + xi) * (ONE + eta) * cz
    N(4) = 0.25_wp * (ONE - xi) * (ONE + eta) * cz
    N(5) = zeta
    dNdxi(1, 1) = -0.25_wp * (ONE - eta) * cz
    dNdxi(2, 1) = -0.25_wp * (ONE - xi) * cz
    dNdxi(3, 1) = -0.25_wp * (ONE - xi) * (ONE - eta)
    dNdxi(1, 2) =  0.25_wp * (ONE - eta) * cz
    dNdxi(2, 2) = -0.25_wp * (ONE + xi) * cz
    dNdxi(3, 2) = -0.25_wp * (ONE + xi) * (ONE - eta)
    dNdxi(1, 3) =  0.25_wp * (ONE + eta) * cz
    dNdxi(2, 3) =  0.25_wp * (ONE + xi) * cz
    dNdxi(3, 3) = -0.25_wp * (ONE + xi) * (ONE + eta)
    dNdxi(1, 4) = -0.25_wp * (ONE + eta) * cz
    dNdxi(2, 4) =  0.25_wp * (ONE - xi) * cz
    dNdxi(3, 4) = -0.25_wp * (ONE - xi) * (ONE + eta)
    dNdxi(1, 5) = ZERO
    dNdxi(2, 5) = ZERO
    dNdxi(3, 5) = ONE
  END SUBROUTINE PH_Elem_C3D5_ShapeFunc

  SUBROUTINE PH_Elem_C3D5_Strain(B, u, strain)
    REAL(wp), INTENT(IN)  :: B(6, 15)
    REAL(wp), INTENT(IN)  :: u(15)
    REAL(wp), INTENT(OUT) :: strain(6)
    strain = MATMUL(B, u)
  END SUBROUTINE PH_Elem_C3D5_Strain

  SUBROUTINE PH_Elem_C3D5_Stress(epsilon, D, sigma)
    REAL(wp), INTENT(IN)  :: epsilon(6)
    REAL(wp), INTENT(IN)  :: D(6, 6)
    REAL(wp), INTENT(OUT) :: sigma(6)
    sigma = MATMUL(D, epsilon)
  END SUBROUTINE PH_Elem_C3D5_Stress

  ! --- Sect (merged from PH_Elem_C3D5_Sect) ---
  SUBROUTINE PH_Elem_C3D5_GetCentroid(coords, centroid)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
  ...
  END SUBROUTINE PH_Elem_C3D5_GetCentroid

  SUBROUTINE PH_Elem_C3D5_GetInertiaOrig(coords, rho, I_out)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: rho
    ...
    DO ip = 1, 8
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D5_GetInertiaOrig

  SUBROUTINE PH_Elem_C3D5_GetSectProps(coords, density_in, volume, mass)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: density_in
    REAL(wp), INTENT(OUT) :: volume
    REAL(wp), INTENT(OUT) :: mass
    CALL PH_Elem_C3D5_GetVolume(coords, volume)
    mass = density_in * volume
  END SUBROUTINE PH_Elem_C3D5_GetSectProps

  SUBROUTINE PH_Elem_C3D5_GetVolume(coords, volume)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(OUT) :: volume
    REAL(wp) :: xi(8), eta(8), zeta(8), weights(8)
    REAL(wp) :: N(5), dNdxi(3, 5), J(3, 3), detJ
    ...
    DO ip = 1, 8
      ...
      volume = volume + detJ * weights(ip)
    END DO
  END SUBROUTINE PH_Elem_C3D5_GetVolume

  ! --- Constraints (merged from PH_Elem_C3D5_Constraints) ---
  SUBROUTINE PH_Elem_C3D5_ApplyConstraint(ctype, idof, val, penalty, K_el, F_el)
  ...
  END SUBROUTINE PH_Elem_C3D5_ApplyConstraint

  SUBROUTINE PH_Elem_C3D5_ApplyMPC(c, val, penalty, K_el, F_el)
    ...
    DO i = 1, 15
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D5_ApplyMPC

  ! --- Cont (merged from PH_Elem_C3D5_Cont) ---
  SUBROUTINE PH_Elem_C3D5_FormContactContrib(face_id, xi, eta, zeta, N, n, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: face_id
    REAL(wp), INTENT(IN)  :: xi, eta, zeta
    REAL(wp), INTENT(IN)  :: N(5)
    ...
    DO a = 1, 5
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D5_FormContactContrib

  SUBROUTINE PH_Elem_C3D5_FormContactFaceCtr(face_id, coords, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: face_id
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: gap, penalty
    REAL(wp), INTENT(INOUT) :: K_el(15, 15)
    REAL(wp), INTENT(INOUT) :: F_el(15)
    REAL(wp) :: xi, eta, zeta, N(5), n(3), dNdxi(3, 5)
    REAL(wp) :: r1(3), r2(3)
    INTEGER(i4) :: nodes(4), i, nnode_face
    IF (face_id < 1 .OR. face_id > 5) RETURN
    nodes(1:4) = PH_ELEM_C3D5_FACE_NODES(1:4, face_id)
    nnode_face = 3
    ...
    ELSE
      ...
      zeta = ZERO
    END IF
    CALL PH_Elem_C3D5_ShapeFunc(xi, eta, zeta, N, dNdxi)
    ...
    CALL PH_Elem_C3D5_FormContactContrib(face_id, xi, eta, zeta, N, n, gap, penalty, K_el, F_el)
  END SUBROUTINE PH_Elem_C3D5_FormContactFaceCtr

  ! --- Loads (merged from PH_Elem_C3D5_Loads) ---
  SUBROUTINE PH_Elem_C3D5_FormFacePressure(coords, p, face_id, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    REAL(wp), INTENT(IN)  :: p
    INTEGER(i4), INTENT(IN)  :: face_id
    ...
    ELSE
      ...
      END DO
    END IF
  END SUBROUTINE PH_Elem_C3D5_FormFacePressure

  SUBROUTINE PH_Elem_C3D5_FormBodyForce(coords, bx, by, bz, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 5)
    ...
    DO ip = 1, 8
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D5_FormBodyForce

  SUBROUTINE PH_Elem_C3D5_FormNodalForce(load_type, coords, val, face_id, F_eq)
    ...
    IF (load_type == PH_ELEM_LOAD_BODY) THEN
      CALL PH_Elem_C3D5_FormBodyForce(coords, val(1), val(2), val(3), F_eq)
    ELSE IF (load_type == PH_ELEM_LOAD_FACE_P .AND. SIZE(val) >= 1) THEN
      CALL PH_Elem_C3D5_FormFacePressure(coords, val(1), face_id, F_eq)
    END IF
  END SUBROUTINE PH_Elem_C3D5_FormNodalForce

  ! --- Out (merged from PH_Elem_C3D5_Out) ---
  SUBROUTINE PH_Elem_C3D5_CollectIPVars(ip_stress, ip_strain, ip_peeq, n_ip, out_vars)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D5_CollectIPVars

  SUBROUTINE PH_Elem_C3D5_EvalPrincStress(sigma, principal)
    ...
    DO i = 1, 2
      ...
      END IF
    END DO
  END SUBROUTINE PH_Elem_C3D5_EvalPrincStress

  SUBROUTINE PH_Elem_C3D5_EvalStrainInvar(strain, I1e, J2e)
  ...
  END SUBROUTINE PH_Elem_C3D5_EvalStrainInvar

  SUBROUTINE PH_Elem_C3D5_EvalStressInvar(sigma, I1, J2, J3)
    REAL(wp), INTENT(IN)  :: sigma(6)
  ...
  END SUBROUTINE PH_Elem_C3D5_EvalStressInvar

  SUBROUTINE PH_Elem_C3D5_EvalTriaxiality(sigma, triax)
    REAL(wp), INTENT(IN)  :: sigma(6)
    REAL(wp), INTENT(OUT) :: triax
    REAL(wp) :: I1, J2, J3, p, seq
    CALL PH_Elem_C3D5_EvalStressInvar(sigma, I1, J2, J3)
    p = -I1 / 3.0_wp
    seq = SQRT(3.0_wp * MAX(J2, ZERO))
    triax = MERGE(p / seq, ZERO, seq > 1.0e-20_wp)
  END SUBROUTINE PH_Elem_C3D5_EvalTriaxiality

  SUBROUTINE PH_Elem_C3D5_EvalVonMises(sigma, seq)
    REAL(wp), INTENT(IN)  :: sigma(6)
    ...
    seq = SQRT(3.0_wp * MAX(J2, ZERO))
  END SUBROUTINE PH_Elem_C3D5_EvalVonMises

  SUBROUTINE PH_Elem_C3D5_GetExtrapMat(E)
    ...
    E(5, 5) = ONE
    E(6, 6) = ONE
  END SUBROUTINE PH_Elem_C3D5_GetExtrapMat

  SUBROUTINE PH_Elem_C3D5_MapToNode(ip_vars, weights, node_vars)
    REAL(wp), INTENT(IN)  :: ip_vars(:, :), weights(:)
    REAL(wp), INTENT(OUT) :: node_vars(:, :)
    INTEGER(i4) :: ic, i, n_comp
    INTEGER(i4), PARAMETER :: PH_ELEM_N_NODE = 5
    node_vars = ZERO
    ...
    DO ic = 1, n_comp
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D5_MapToNode

  SUBROUTINE PH_Elem_C3D5_Material_Update_Routed(rt_ctx, mat_slot, dStrain, &
    ...
    CALL PH_Elem_MatRoute_Elastic3D(rt_ctx, mat_slot, dStrain, &
                                    stress_old, stress_new, D_tangent, status)
  END SUBROUTINE PH_Elem_C3D5_Material_Update_Routed

END MODULE PH_Elem_C3D5
