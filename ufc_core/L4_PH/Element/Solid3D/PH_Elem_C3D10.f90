!===============================================================================
! MODULE: PH_Elem_C3D10
! LAYER:  L4_PH
! DOMAIN: Element/Solid3D
! ROLE:   Proc
! BRIEF:  C3D10 element definition (10-node quadratic tetrahedron)
!===============================================================================
MODULE PH_Elem_C3D10
...
!> Merged: Defn + Sect + Constraints + Cont + Loads + Out
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: PH_Elem_C3D10_DefInit
  PUBLIC :: PH_Elem_C3D10_ShapeFunc
  PUBLIC :: PH_Elem_C3D10_Jac
  PUBLIC :: PH_Elem_C3D10_BMatrix
  PUBLIC :: PH_Elem_C3D10_GaussPoints
  PUBLIC :: PH_Elem_C3D10_JacB
  PUBLIC :: PH_Elem_C3D10_Strain
  PUBLIC :: PH_Elem_C3D10_Stress
  PUBLIC :: PH_Elem_C3D10_ConstMatrix
  PUBLIC :: PH_Elem_C3D10_FormStiffMatrix
  PUBLIC :: PH_Elem_C3D10_FormStiffMatrixFromD
  PUBLIC :: PH_Elem_C3D10_FormIntForce
  PUBLIC :: PH_Elem_C3D10_FormIntForceFromStress
  PUBLIC :: PH_Elem_C3D10_ConsMass
  PUBLIC :: PH_Elem_C3D10_LumpMass
  PUBLIC :: PH_Elem_C3D10_ThermStrainVector
  PUBLIC :: PH_ELEM_C3D10_NNODE
  PUBLIC :: PH_ELEM_C3D10_NIP
  PUBLIC :: PH_ELEM_C3D10_NDOF
  PUBLIC :: PH_ELEM_C3D10_NFACE
  PUBLIC :: PH_ELEM_C3D10_FACE_NODES
  PUBLIC :: PH_Elem_C3D10_NL_TL
  PUBLIC :: PH_Elem_C3D10_NL_UL
  PUBLIC :: PH_Elem_C3D10_GetVolume, PH_Elem_C3D10_GetSectProps, PH_Elem_C3D10_GetCentroid
  PUBLIC :: PH_Elem_C3D10_GetInertiaOrig
  PUBLIC :: PH_Elem_C3D10_ApplyConstraint, PH_Elem_C3D10_ApplyMPC
  PUBLIC :: PH_Elem_C3D10_FormContactContrib, PH_Elem_C3D10_FormContactFaceCtr
  PUBLIC :: PH_Elem_C3D10_FormNodalForce, PH_Elem_C3D10_FormBodyForce, PH_Elem_C3D10_FormFacePressure
  PUBLIC :: PH_Elem_C3D10_CollectIPVars, PH_Elem_C3D10_MapToNode, PH_Elem_C3D10_GetExtrapMat
  PUBLIC :: PH_Elem_C3D10_EvalVonMises, PH_Elem_C3D10_EvalPrincStress
  PUBLIC :: PH_Elem_C3D10_EvalStressInvar, PH_Elem_C3D10_EvalStrainInvar, PH_Elem_C3D10_EvalTriaxiality
  PUBLIC :: PH_Elem_C3D10_Material_Update_Routed
  PUBLIC :: PH_ELEM_CTYPE_PENALTY_DOF, PH_ELEM_CTYPE_MPC_LINEAR
  PUBLIC :: PH_ELEM_LOAD_BODY, PH_ELEM_LOAD_FACE_P

  INTEGER(i4), PARAMETER :: PH_ELEM_C3D10_NNODE  = 10_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D10_NIP   = 4_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D10_NDOF  = 30_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D10_NFACE = 4_i4
  REAL(wp), PARAMETER :: TET_A = 0.585410196624968_wp
  REAL(wp), PARAMETER :: TET_B = 0.138196601125011_wp
  REAL(wp), PARAMETER :: TET_W = 1.0_wp / 24.0_wp
  ! Face k opposite corner k: 3 corners + 3 edge mids. (2,3,4, 6,10,8), (1,3,4, 7,10,9), (1,2,4, 5,9,8), (1,2,3, 5,6,7)
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D10_FACE_NODES(6, 4) = RESHAPE([ &
    2,3,4, 6,10,8, 1,3,4, 7,10,9, 1,2,4, 5,9,8, 1,2,3, 5,6,7 ], [6, 4])
...
CONTAINS
  ...
  SUBROUTINE PH_Elem_C3D10_ThermStrainVector(alpha, deltaT, eps_th)
    REAL(wp), INTENT(IN)  :: alpha, deltaT
    REAL(wp), INTENT(OUT) :: eps_th(6)
    eps_th(1:3) = alpha * deltaT
    eps_th(4:6) = ZERO
  END SUBROUTINE PH_Elem_C3D10_ThermStrainVector

  SUBROUTINE PH_Elem_C3D10_BMatrix(dNdx, B)
    REAL(wp), INTENT(IN)  :: dNdx(3, 10)
    ...
    DO i = 1, 10
      ...
      B(6, c+2) = dNdx(1, i)
    END DO
  END SUBROUTINE PH_Elem_C3D10_BMatrix

  SUBROUTINE PH_Elem_C3D10_ConsMass(coords, rho, Me)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: rho
    REAL(wp), INTENT(OUT) :: Me(30, 30)
    ...
    DO ip = 1, 4
      CALL PH_Elem_C3D10_ShapeFunc(xi(ip), eta(ip), zeta(ip), N, dNdxi)
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D10_ConsMass

  SUBROUTINE PH_Elem_C3D10_ConstMatrix(E_young, nu, D)
    ...
    D(5,5) = D(4,4)
    D(6,6) = D(4,4)
  END SUBROUTINE PH_Elem_C3D10_ConstMatrix

  SUBROUTINE PH_Elem_C3D10_DefInit()
  END SUBROUTINE PH_Elem_C3D10_DefInit

  SUBROUTINE PH_Elem_C3D10_FormIntForce(coords, u, E_young, nu, R_int)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: u(30)
    REAL(wp), INTENT(IN)  :: E_young, nu
    REAL(wp), INTENT(OUT) :: R_int(30)
    REAL(wp) :: Ke(30, 30)
    CALL PH_Elem_C3D10_FormStiffMatrix(coords, E_young, nu, Ke)
    R_int = MATMUL(Ke, u)
  END SUBROUTINE PH_Elem_C3D10_FormIntForce

  SUBROUTINE PH_Elem_C3D10_FormIntForceFromStress(coords, sigma6, R_int)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D10_FormIntForceFromStress

  SUBROUTINE PH_Elem_C3D10_FormStiffMatrix(coords, E_young, nu, Ke)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: E_young, nu
    REAL(wp), INTENT(OUT) :: Ke(30, 30)
    REAL(wp) :: N(10), dNdx(3, 10), J(3, 3), detJ, B(6, 30), D(6, 6)
    REAL(wp) :: xi(4), eta(4), zeta(4), weights(4)
    INTEGER(i4) :: ip
    Ke = ZERO
    CALL PH_Elem_C3D10_ConstMatrix(E_young, nu, D)
    CALL PH_Elem_C3D10_GaussPoints(xi, eta, zeta, weights)
    DO ip = 1, 4
      CALL PH_Elem_C3D10_JacB(coords, xi(ip), eta(ip), zeta(ip), N, dNdx, J, detJ, B)
      IF (ABS(detJ) <= 1.0e-12_wp) CYCLE
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D10_FormStiffMatrix

  SUBROUTINE PH_Elem_C3D10_FormStiffMatrixFromD(coords, D6, Ke)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D10_FormStiffMatrixFromD

  SUBROUTINE PH_Elem_C3D10_GaussPoints(xi, eta, zeta, weights)
    ...
    zeta(4) = TET_B
    weights(1:4) = TET_W
  END SUBROUTINE PH_Elem_C3D10_GaussPoints

  SUBROUTINE PH_Elem_C3D10_GetVolumeInt(coords, volume)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D10_GetVolumeInt

  SUBROUTINE PH_Elem_C3D10_Jac(dNdxi, coords, J, detJ)
    REAL(wp), INTENT(IN)  :: dNdxi(3, 10)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
  ...
  END SUBROUTINE PH_Elem_C3D10_Jac

  SUBROUTINE PH_Elem_C3D10_JacB(coords, xi_pt, eta_pt, zeta_pt, N, dNdx, J, detJ, B)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: xi_pt, eta_pt, zeta_pt
    REAL(wp), INTENT(OUT) :: N(10)
    REAL(wp), INTENT(OUT) :: dNdx(3, 10)
    REAL(wp), INTENT(OUT) :: J(3, 3)
    REAL(wp), INTENT(OUT) :: detJ
    REAL(wp), INTENT(OUT) :: B(6, 30)
    REAL(wp) :: dNdxi(3, 10), J_inv(3, 3)
    INTEGER(i4) :: i, j
    CALL PH_Elem_C3D10_ShapeFunc(xi_pt, eta_pt, zeta_pt, N, dNdxi)
    CALL PH_Elem_C3D10_Jac(dNdxi, coords, J, detJ)
    ...
    DO i = 1, 10
      ...
      END DO
    END DO
    CALL PH_Elem_C3D10_BMatrix(dNdx, B)
  END SUBROUTINE PH_Elem_C3D10_JacB

  SUBROUTINE PH_Elem_C3D10_LumpMass(coords, rho, M_lumped)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: rho
    REAL(wp), INTENT(OUT) :: M_lumped(30)
    REAL(wp) :: V, m
    INTEGER(i4) :: i
    CALL PH_Elem_C3D10_GetVolumeInt(coords, V)
    m = rho * V / 10.0_wp
    DO i = 1, 10
      M_lumped(3*i-2) = m
      M_lumped(3*i-1) = m
      M_lumped(3*i)   = m
    END DO
  END SUBROUTINE PH_Elem_C3D10_LumpMass

  SUBROUTINE PH_Elem_C3D10_NL_TL(coords_ref, u_elem, mat_prop, mat_state, Ke_mat, Ke_geo, R_int, status)
    REAL(wp), INTENT(IN) :: coords_ref(3, 10)
    REAL(wp), INTENT(IN) :: u_elem(30)
    TYPE(MatPropertyDef), INTENT(IN) :: mat_prop
    TYPE(PH_MatPoint_State), INTENT(INOUT) :: mat_state(:)
    REAL(wp), INTENT(OUT) :: Ke_mat(30, 30)
    REAL(wp), INTENT(OUT) :: Ke_geo(30, 30)
    REAL(wp), INTENT(OUT) :: R_int(30)
    TYPE(ErrorStatusType), INTENT(OUT) :: status

    REAL(wp) :: coords_curr(3, 10)
    REAL(wp) :: xi_gp(4), eta_gp(4), zeta_gp(4), wt_gp(4)
    REAL(wp) :: N(10), dN_dxi(3, 10), dN_dX(3, 10)
    ...
    REAL(wp) :: K_mat_gp(30, 30), K_geo_gp(30, 30), R_gp(30)
    TYPE(RT_LagrCfg) :: cfg
    INTEGER(i4) :: i, igp, j
    ...
    CALL PH_Elem_C3D10_GaussPoints(xi_gp, eta_gp, zeta_gp, wt_gp)
    ...
    DO igp = 1, PH_ELEM_C3D10_NIP
      CALL PH_Elem_C3D10_ShapeFunc(xi_gp(igp), eta_gp(igp), zeta_gp(igp), N, dN_dxi)
      CALL PH_Elem_C3D10_Jac(dN_dxi, coords_ref, J_ref, det_J)
  ...
  END SUBROUTINE PH_Elem_C3D10_NL_TL

  SUBROUTINE PH_Elem_C3D10_NL_UL(coords_prev, u_incr, mat_prop, mat_state, Ke_mat, Ke_geo, R_int, status)
    REAL(wp), INTENT(IN) :: coords_prev(3, 10)
    REAL(wp), INTENT(IN) :: u_incr(30)
    TYPE(MatPropertyDef), INTENT(IN) :: mat_prop
    TYPE(PH_MatPoint_State), INTENT(INOUT) :: mat_state(:)
    REAL(wp), INTENT(OUT) :: Ke_mat(30, 30)
    REAL(wp), INTENT(OUT) :: Ke_geo(30, 30)
    ...
    DO igp = 1, PH_ELEM_C3D10_NIP
      CALL PH_Elem_C3D10_ShapeFunc(xi_gp(igp), eta_gp(igp), zeta_gp(igp), N, dN_dxi)
      CALL PH_Elem_C3D10_Jac(dN_dxi, coords_prev, J_prev, det_J)
  ...
  CONTAINS
    SUBROUTINE Invert3x3(A, A_inv, det)
      ...
      A_inv(3,3) = (A(1,1)*A(2,2)-A(1,2)*A(2,1))/det
    END SUBROUTINE Invert3x3
  END SUBROUTINE PH_Elem_C3D10_NL_UL

  SUBROUTINE PH_Elem_C3D10_Material_Update_Routed(rt_ctx, mat_slot, dStrain, &
    ...
    CALL PH_Elem_MatRoute_Elastic3D(rt_ctx, mat_slot, dStrain, &
                                    stress_old, stress_new, D_tangent, status)
  END SUBROUTINE PH_Elem_C3D10_Material_Update_Routed

  SUBROUTINE PH_Elem_C3D10_ShapeFunc(xi, eta, zeta, N, dNdxi)
    REAL(wp), INTENT(IN)  :: xi, eta, zeta
    REAL(wp), INTENT(OUT) :: N(10)
    REAL(wp), INTENT(OUT) :: dNdxi(3, 10)
    REAL(wp) :: L1, L2, L3, L4
    L1 = xi
    L2 = eta
    L3 = zeta
    ...
    N(9) = 4.0_wp * L2 * L4
    N(10) = 4.0_wp * L3 * L4
    dNdxi(1, 1) = (4.0_wp*L1 - ONE)
    dNdxi(2, 1) = ZERO
    dNdxi(3, 1) = ZERO
    dNdxi(1, 2) = ZERO
    dNdxi(2, 2) = (4.0_wp*L2 - ONE)
    dNdxi(3, 2) = ZERO
    dNdxi(1, 3) = ZERO
    dNdxi(2, 3) = ZERO
    dNdxi(3, 3) = (4.0_wp*L3 - ONE)
    dNdxi(1, 4) = -(4.0_wp*L4 - ONE)
    dNdxi(2, 4) = -(4.0_wp*L4 - ONE)
    dNdxi(3, 4) = -(4.0_wp*L4 - ONE)
    dNdxi(1, 5) = 4.0_wp * L2
    dNdxi(2, 5) = 4.0_wp * L1
    dNdxi(3, 5) = ZERO
    dNdxi(1, 6) = ZERO
    ...
    dNdxi(2, 7) = ZERO
    dNdxi(3, 7) = 4.0_wp * L1
    dNdxi(1, 8) = 4.0_wp * (L4 - L1)
    dNdxi(2, 8) = -4.0_wp * L1
    dNdxi(3, 8) = -4.0_wp * L1
    dNdxi(1, 9) = -4.0_wp * L2
    dNdxi(2, 9) = 4.0_wp * (L4 - L2)
    dNdxi(3, 9) = -4.0_wp * L2
    dNdxi(1, 10) = -4.0_wp * L3
    dNdxi(2, 10) = -4.0_wp * L3
    dNdxi(3, 10) = 4.0_wp * (L4 - L3)
  END SUBROUTINE PH_Elem_C3D10_ShapeFunc

  SUBROUTINE PH_Elem_C3D10_Strain(B, u, strain)
    REAL(wp), INTENT(IN)  :: B(6, 30)
    REAL(wp), INTENT(IN)  :: u(30)
    REAL(wp), INTENT(OUT) :: strain(6)
    strain = MATMUL(B, u)
  END SUBROUTINE PH_Elem_C3D10_Strain

  SUBROUTINE PH_Elem_C3D10_Stress(epsilon, D, sigma)
    REAL(wp), INTENT(IN)  :: epsilon(6)
    REAL(wp), INTENT(IN)  :: D(6, 6)
    REAL(wp), INTENT(OUT) :: sigma(6)
    sigma = MATMUL(D, epsilon)
  END SUBROUTINE PH_Elem_C3D10_Stress

  ! --- Sect (merged from PH_Elem_C3D10_Sect) ---
  SUBROUTINE PH_Elem_C3D10_GetCentroid(coords, centroid)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
  ...
  END SUBROUTINE PH_Elem_C3D10_GetCentroid

  SUBROUTINE PH_Elem_C3D10_GetInertiaOrig(coords, rho, I_out)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: rho
    ...
    DO ip = 1, 4
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D10_GetInertiaOrig

  SUBROUTINE PH_Elem_C3D10_GetSectProps(coords, density_in, volume, mass)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: density_in
    REAL(wp), INTENT(OUT) :: volume
    REAL(wp), INTENT(OUT) :: mass
    CALL PH_Elem_C3D10_GetVolume(coords, volume)
    mass = density_in * volume
  END SUBROUTINE PH_Elem_C3D10_GetSectProps

  SUBROUTINE PH_Elem_C3D10_GetVolume(coords, volume)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(OUT) :: volume
    REAL(wp) :: xi(4), eta(4), zeta(4), weights(4)
    REAL(wp) :: N(10), dNdxi(3, 10), J(3, 3), detJ
    ...
    DO ip = 1, 4
      ...
      volume = volume + detJ * weights(ip)
    END DO
  END SUBROUTINE PH_Elem_C3D10_GetVolume

  ! --- Constraints (merged from PH_Elem_C3D10_Constraints) ---
  SUBROUTINE PH_Elem_C3D10_ApplyConstraint(ctype, idof, val, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)    :: ctype
    INTEGER(i4), INTENT(IN)    :: idof
    REAL(wp), INTENT(IN)    :: val
    REAL(wp), INTENT(IN)    :: penalty
    REAL(wp), INTENT(INOUT) :: K_el(30, 30)
    REAL(wp), INTENT(INOUT) :: F_el(30)
    IF (ctype /= PH_ELEM_CTYPE_PENALTY_DOF) RETURN
    IF (idof < 1 .OR. idof > 30) RETURN
    K_el(idof, idof) = K_el(idof, idof) + penalty
    F_el(idof) = F_el(idof) + penalty * val
  END SUBROUTINE PH_Elem_C3D10_ApplyConstraint

  SUBROUTINE PH_Elem_C3D10_ApplyMPC(c, val, penalty, K_el, F_el)
    REAL(wp), INTENT(IN)    :: c(30)
    REAL(wp), INTENT(IN)    :: val
    REAL(wp), INTENT(IN)    :: penalty
    REAL(wp), INTENT(INOUT) :: K_el(30, 30)
    REAL(wp), INTENT(INOUT) :: F_el(30)
    ...
    DO i = 1, 30
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D10_ApplyMPC

  ! --- Cont (merged from PH_Elem_C3D10_Cont) ---
  SUBROUTINE PH_Elem_C3D10_FormContactContrib(face_id, xi, eta, zeta, N, n, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: face_id
    REAL(wp), INTENT(IN)  :: xi, eta, zeta
    REAL(wp), INTENT(IN)  :: N(10)
    ...
    DO a = 1, 10
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D10_FormContactContrib

  SUBROUTINE PH_Elem_C3D10_FormContactFaceCtr(face_id, coords, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: face_id
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: gap, penalty
    REAL(wp), INTENT(INOUT) :: K_el(30, 30)
    REAL(wp), INTENT(INOUT) :: F_el(30)
    REAL(wp) :: xi, eta, zeta, N(10), n(3), dNdxi(3, 10)
    REAL(wp) :: r1(3), r2(3)
    INTEGER(i4) :: nodes(6)
    ...
    eta = 1.0_wp/3.0_wp
    zeta = 1.0_wp/3.0_wp
    IF (face_id == 1) zeta = 0.0_wp
    IF (face_id == 2) eta = 0.0_wp
    IF (face_id == 3) xi = 0.0_wp
    IF (face_id == 4) THEN
      xi = 0.0_wp
      eta = 0.0_wp
      zeta = 0.0_wp
    ...
    r2(1:3) = coords(1:3, nodes(3)) - coords(1:3, nodes(1))
    n(1) = r1(2)*r2(3) - r1(3)*r2(2)
    n(2) = r1(3)*r2(1) - r1(1)*r2(3)
    n(3) = r1(1)*r2(2) - r1(2)*r2(1)
    IF (face_id == 1) n = -n
    IF (SUM(n**2) > 1.0e-20_wp) n = n / SQRT(SUM(n**2))
    CALL PH_Elem_C3D10_FormContactContrib(face_id, xi, eta, zeta, N, n, gap, penalty, K_el, F_el)
  END SUBROUTINE PH_Elem_C3D10_FormContactFaceCtr

  ! --- Loads (merged from PH_Elem_C3D10_Loads) ---
  SUBROUTINE PH_Elem_C3D10_FormFacePressure(coords, p, face_id, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    REAL(wp), INTENT(IN)  :: p
    INTEGER(i4), INTENT(IN)  :: face_id
    REAL(wp), INTENT(OUT) :: F_eq(30)
    REAL(wp) :: n(3), area
    ...
    IF (face_id == 1) n = -n
    DO i = 1, 6
      F_eq(3*nodes(i)-2:3*nodes(i)) = F_eq(3*nodes(i)-2:3*nodes(i)) + (p * area / 6.0_wp) * n(1:3)
    END DO
  END SUBROUTINE PH_Elem_C3D10_FormFacePressure

  SUBROUTINE PH_Elem_C3D10_FormBodyForce(coords, bx, by, bz, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 10)
    ...
    DO ip = 1, 4
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D10_FormBodyForce

  SUBROUTINE PH_Elem_C3D10_FormNodalForce(load_type, coords, val, face_id, F_eq)
    ...
    IF (load_type == PH_ELEM_LOAD_BODY) THEN
      CALL PH_Elem_C3D10_FormBodyForce(coords, val(1), val(2), val(3), F_eq)
    ELSE IF (load_type == PH_ELEM_LOAD_FACE_P .AND. SIZE(val) >= 1) THEN
      CALL PH_Elem_C3D10_FormFacePressure(coords, val(1), face_id, F_eq)
    END IF
  END SUBROUTINE PH_Elem_C3D10_FormNodalForce

  ! --- Out (merged from PH_Elem_C3D10_Out) ---
  SUBROUTINE PH_Elem_C3D10_CollectIPVars(ip_stress, ip_strain, ip_peeq, n_ip, out_vars)
    REAL(wp), INTENT(IN)  :: ip_stress(:, :), ip_strain(:, :), ip_peeq(:)
    INTEGER(i4), INTENT(IN)  :: n_ip
    REAL(wp), INTENT(OUT) :: out_vars(:, :)
    INTEGER(i4) :: ip, nv
    INTEGER(i4), PARAMETER :: PH_ELEM_N_IP = 4
    nv = 13
    out_vars = ZERO
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D10_CollectIPVars

  SUBROUTINE PH_Elem_C3D10_EvalPrincStress(sigma, principal)
    ...
    DO i = 1, 2
      ...
      END IF
    END DO
  END SUBROUTINE PH_Elem_C3D10_EvalPrincStress

  SUBROUTINE PH_Elem_C3D10_EvalStrainInvar(strain, I1e, J2e)
  ...
  END SUBROUTINE PH_Elem_C3D10_EvalStrainInvar

  SUBROUTINE PH_Elem_C3D10_EvalStressInvar(sigma, I1, J2, J3)
    REAL(wp), INTENT(IN)  :: sigma(6)
  ...
  END SUBROUTINE PH_Elem_C3D10_EvalStressInvar

  SUBROUTINE PH_Elem_C3D10_EvalTriaxiality(sigma, triax)
    REAL(wp), INTENT(IN)  :: sigma(6)
    REAL(wp), INTENT(OUT) :: triax
    REAL(wp) :: I1, J2, J3, p, seq
    CALL PH_Elem_C3D10_EvalStressInvar(sigma, I1, J2, J3)
    p = -I1 / 3.0_wp
    seq = SQRT(3.0_wp * MAX(J2, ZERO))
    triax = MERGE(p / seq, ZERO, seq > 1.0e-20_wp)
  END SUBROUTINE PH_Elem_C3D10_EvalTriaxiality

  SUBROUTINE PH_Elem_C3D10_EvalVonMises(sigma, seq)
    REAL(wp), INTENT(IN)  :: sigma(6)
    ...
    seq = SQRT(3.0_wp * MAX(J2, ZERO))
  END SUBROUTINE PH_Elem_C3D10_EvalVonMises

  SUBROUTINE PH_Elem_C3D10_GetExtrapMat(E)
    ...
    E(3, 3) = ONE
    E(4, 4) = ONE
  END SUBROUTINE PH_Elem_C3D10_GetExtrapMat

  SUBROUTINE PH_Elem_C3D10_MapToNode(ip_vars, weights, node_vars)
    REAL(wp), INTENT(IN)  :: ip_vars(:, :), weights(:)
    REAL(wp), INTENT(OUT) :: node_vars(:, :)
    INTEGER(i4) :: ic, i, n_comp
    INTEGER(i4), PARAMETER :: PH_ELEM_N_NODE = 10
    node_vars = ZERO
    ...
    DO ic = 1, n_comp
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D10_MapToNode
END MODULE PH_Elem_C3D10
