!===============================================================================
! MODULE: PH_ShapeScalarField
! LAYER:  L4_PH
! DOMAIN: Element
! ROLE:   Proc
! BRIEF:  Unified N, dNdx, detJ for scalar-field assembly (heat, electric,
!===============================================================================
MODULE PH_ShapeScalarField
...
CONTAINS
  ...
  SUBROUTINE RT_Asm_ShapeScalarField_Eval(elem_type_id, coords, npe, ip, N, dNdx, detJ, weight, status)
    ...
    CASE DEFAULT
      ...
      status%message = 'PH_ShapeScalarField: unsupported elem_type'
      RETURN
    END SELECT
    status%status_code = IF_STATUS_OK
  END SUBROUTINE RT_Asm_ShapeScalarField_Eval

END MODULE PH_ShapeScalarField
