...
! ROLE:   Proc
! BRIEF:  T3D2 2-node 3D truss element
!===============================================================================
MODULE PH_Elem_T3D2
!> [CORE] T3D2 2-node 3D truss (merged Defn+Sect+Constraints+Cont+Loads+Out)
  ...
  INTEGER(i4), PARAMETER :: PH_ELEM_T3D2_NNODE  = 2_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_T3D2_NIP   = 1_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_T3D2_NDOF  = 6_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_T3D2_NEDGE = 0_i4
  ...
  PUBLIC :: PH_Elem_T3D2_DefInit
  PUBLIC :: PH_Elem_T3D2_ShapeFunc
  PUBLIC :: PH_Elem_T3D2_BMatrix
  PUBLIC :: PH_Elem_T3D2_FormStiffMatrix
  PUBLIC :: PH_Elem_T3D2_FormIntForce
  PUBLIC :: PH_Elem_T3D2_ConsMass
  PUBLIC :: PH_Elem_T3D2_LumpMass
  PUBLIC :: PH_Elem_T3D2_ThermStrainVector
  PUBLIC :: PH_Elem_T3D2_NL_TL
  PUBLIC :: PH_Elem_T3D2_NL_UL
  PUBLIC :: UF_Elem_T3D2_Calc
  PUBLIC :: PH_Elem_T3D2_GetArea
  PUBLIC :: PH_Elem_T3D2_GetLength
  PUBLIC :: PH_Elem_T3D2_GetSectProps
  PUBLIC :: PH_Elem_T3D2_GetCentroid
  PUBLIC :: PH_Elem_T3D2_ApplyConstraint
  PUBLIC :: PH_Elem_T3D2_ApplyMPC
  PUBLIC :: PH_Elem_T3D2_FormContactContrib
  PUBLIC :: PH_Elem_T3D2_FormContactEdgeCtr
  PUBLIC :: PH_Elem_T3D2_FormNodalForce
  PUBLIC :: PH_Elem_T3D2_FormBodyForce
  PUBLIC :: PH_Elem_T3D2_CollectIPVars
  PUBLIC :: PH_Elem_T3D2_MapToNode
  PUBLIC :: PH_Elem_T3D2_GetExtrapMat
  PUBLIC :: PH_Elem_T3D2_EvalAxialStress
  PUBLIC :: PH_Elem_T3D2_Material_Update_Routed
  PUBLIC :: PH_ELEM_T3D2_NNODE, PH_ELEM_T3D2_NIP, PH_ELEM_T3D2_NDOF, PH_ELEM_T3D2_NEDGE
...
CONTAINS
  ...
  ! DEFINITION
  !=============================================================================
  SUBROUTINE PH_Elem_T3D2_ThermStrainVector(alpha, deltaT, eps_th)
    REAL(wp), INTENT(IN)  :: alpha, deltaT
    REAL(wp), INTENT(OUT) :: eps_th(1)
    eps_th(1) = alpha * deltaT
  END SUBROUTINE PH_Elem_T3D2_ThermStrainVector

  SUBROUTINE PH_Elem_T3D2_BMatrix(coords, L, e_dir, B)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    REAL(wp), INTENT(IN)  :: L
    REAL(wp), INTENT(OUT) :: e_dir(3)
    REAL(wp), INTENT(OUT) :: B(1, 6)
    REAL(wp) :: invL
    B = ZERO
    ...
    e_dir(3) = (coords(3, 2) - coords(3, 1)) * invL
    B(1, 1) = -e_dir(1) * invL
    B(1, 2) = -e_dir(2) * invL
    B(1, 3) = -e_dir(3) * invL
    B(1, 4) =  e_dir(1) * invL
    B(1, 5) =  e_dir(2) * invL
    B(1, 6) =  e_dir(3) * invL
  END SUBROUTINE PH_Elem_T3D2_BMatrix

  SUBROUTINE PH_Elem_T3D2_ConsMass(coords, rho, area, Me)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    REAL(wp), INTENT(IN)  :: rho, area
    REAL(wp), INTENT(OUT) :: Me(6, 6)
    REAL(wp) :: dx, dy, dz, L, m
    Me = ZERO
    ...
    Me(5, 5) = m
    Me(6, 6) = m
  END SUBROUTINE PH_Elem_T3D2_ConsMass

  SUBROUTINE PH_Elem_T3D2_DefInit()
  END SUBROUTINE PH_Elem_T3D2_DefInit

  SUBROUTINE PH_Elem_T3D2_FormIntForce(coords, u, E_young, area, R_int)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    REAL(wp), INTENT(IN)  :: u(6)
    REAL(wp), INTENT(IN)  :: E_young, area
    REAL(wp), INTENT(OUT) :: R_int(6)
    REAL(wp) :: L, e_dir(3), B(1, 6), strain, sigma, vol
    REAL(wp) :: dx, dy, dz
    R_int = ZERO
    dx = coords(1, 2) - coords(1, 1)
    dy = coords(2, 2) - coords(2, 1)
    ...
    L = SQRT(dx*dx + dy*dy + dz*dz)
    IF (L <= 1.0e-12_wp) RETURN
    CALL PH_Elem_T3D2_BMatrix(coords, L, e_dir, B)
    strain = MATMUL(B, u)
    sigma = E_young * strain
    vol = area * L
    R_int = MATMUL(TRANSPOSE(B), [sigma]) * vol
  END SUBROUTINE PH_Elem_T3D2_FormIntForce

  SUBROUTINE PH_Elem_T3D2_FormStiffMatrix(coords, E_young, area, Ke)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    REAL(wp), INTENT(IN)  :: E_young, area
    REAL(wp), INTENT(OUT) :: Ke(6, 6)
    REAL(wp) :: dx, dy, dz, L, e_dir(3), B(1, 6), EA_L
    Ke = ZERO
    dx = coords(1, 2) - coords(1, 1)
    dy = coords(2, 2) - coords(2, 1)
    dz = coords(3, 2) - coords(3, 1)
  ...
  END SUBROUTINE PH_Elem_T3D2_FormStiffMatrix

  SUBROUTINE PH_Elem_T3D2_LumpMass(coords, rho, area, M_lumped)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    REAL(wp), INTENT(IN)  :: rho, area
    ...
    M_lumped(1:3) = m
    M_lumped(4:6) = m
  END SUBROUTINE PH_Elem_T3D2_LumpMass

  SUBROUTINE PH_Elem_T3D2_NL_TL(coords_ref, u_elem, E_young, area, &
    ...
    IF (L_ref <= 1.0e-12_wp) THEN
      ...
      status%msg = "PH_ELEM_T3D2_NL_TL: Zero reference length"
      RETURN
    END IF
  ...
  END SUBROUTINE PH_Elem_T3D2_NL_TL

  SUBROUTINE PH_Elem_T3D2_NL_UL(coords_prev, u_incr, E_young, area, &
                                  Ke_mat, Ke_geo, R_int, status)
    ...
    IF (L_prev <= 1.0e-12_wp) THEN
      status%code = IF_STATUS_ERROR
      status%msg = "PH_ELEM_T3D2_NL_UL: Zero previous length"
      RETURN
    END IF
  ...
  END SUBROUTINE PH_Elem_T3D2_NL_UL

  SUBROUTINE PH_Elem_T3D2_ShapeFunc(xi, N)
    REAL(wp), INTENT(IN)  :: xi
    REAL(wp), INTENT(OUT) :: N(2)
    N(1) = (ONE - xi) * HALF
    N(2) = (ONE + xi) * HALF
  END SUBROUTINE PH_Elem_T3D2_ShapeFunc

  SUBROUTINE UF_Elem_T3D2_Calc(ElemType, Formul, Ctx, state_in, &
                                 Mat, state_out, flags)
    ...
    IF (nNode /= 2) THEN
      ...
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_T3D2_Calc: expected 2 nodes')
    ...
    IF (.NOT. ALLOCATED(Ctx%coords_ref)) THEN
      ...
      flags%stableDt = 0.0_wp
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_T3D2_Calc: coords_ref not allocated')
      state_out%failed = flags%failed
      state_out%stableDt = flags%stableDt
      RETURN
    END IF
    ...
    x2 = coords(:, 2)
    dx = x2 - x1
    CALL PH_Elem_T3D2_GetLength(coords, L0)
  ...
  END SUBROUTINE UF_Elem_T3D2_Calc

  !=============================================================================
  ! SECTION
  !=============================================================================
  SUBROUTINE PH_Elem_T3D2_GetArea(area_in, area)
    REAL(wp), INTENT(IN)  :: area_in
    REAL(wp), INTENT(OUT) :: area
    area = area_in
  END SUBROUTINE PH_Elem_T3D2_GetArea

  SUBROUTINE PH_Elem_T3D2_GetCentroid(coords, centroid)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    REAL(wp), INTENT(OUT) :: centroid(3)
    centroid(1) = (coords(1, 1) + coords(1, 2)) * HALF
    centroid(2) = (coords(2, 1) + coords(2, 2)) * HALF
    centroid(3) = (coords(3, 1) + coords(3, 2)) * HALF
  END SUBROUTINE PH_Elem_T3D2_GetCentroid

  SUBROUTINE PH_Elem_T3D2_GetLength(coords, length)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
  ...
  END SUBROUTINE PH_Elem_T3D2_GetLength

  SUBROUTINE PH_Elem_T3D2_GetSectProps(coords, density_in, area_in, length, mass)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    REAL(wp), INTENT(IN)  :: density_in, area_in
    REAL(wp), INTENT(OUT) :: length, mass
    CALL PH_Elem_T3D2_GetLength(coords, length)
    mass = density_in * area_in * length
  END SUBROUTINE PH_Elem_T3D2_GetSectProps

  !=============================================================================
  ! CONSTRAINTS
  !=============================================================================
  SUBROUTINE PH_Elem_T3D2_ApplyConstraint(ctype, idof, val, penalty, K_el, F_el)
  ...
  END SUBROUTINE PH_Elem_T3D2_ApplyConstraint

  SUBROUTINE PH_Elem_T3D2_ApplyMPC(c, val, penalty, K_el, F_el)
    ...
    DO i = 1, 6
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_T3D2_ApplyMPC

  !=============================================================================
  ! CONTACT
  !=============================================================================
  SUBROUTINE PH_Elem_T3D2_FormContactContrib(edge_id, xi, N, n, gap, penalty, edge_len, K_el, F_el)
    ...
    REAL(wp), INTENT(INOUT) :: K_el(6, 6)
    REAL(wp), INTENT(INOUT) :: F_el(6)
  END SUBROUTINE PH_Elem_T3D2_FormContactContrib

  SUBROUTINE PH_Elem_T3D2_FormContactEdgeCtr(edge_id, coords, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: edge_id
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    ...
    K_el = ZERO
    F_el = ZERO
  END SUBROUTINE PH_Elem_T3D2_FormContactEdgeCtr

  !=============================================================================
  ! LOADS
  !=============================================================================
  SUBROUTINE PH_Elem_T3D2_FormBodyForce(coords, bx, by, bz, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 2)
    ...
    w(1) = 2.0_wp
    DO ip = 1, 1
      CALL PH_Elem_T3D2_ShapeFunc(xip(ip), N)
      F_eq(1) = F_eq(1) + N(1) * bx * L * HALF * w(ip)
      F_eq(2) = F_eq(2) + N(1) * by * L * HALF * w(ip)
      F_eq(3) = F_eq(3) + N(1) * bz * L * HALF * w(ip)
      F_eq(4) = F_eq(4) + N(2) * bx * L * HALF * w(ip)
      F_eq(5) = F_eq(5) + N(2) * by * L * HALF * w(ip)
      F_eq(6) = F_eq(6) + N(2) * bz * L * HALF * w(ip)
    END DO
  END SUBROUTINE PH_Elem_T3D2_FormBodyForce

  SUBROUTINE PH_Elem_T3D2_FormNodalForce(load_type, coords, val, edge_id, F_eq)
    ...
    IF (load_type == PH_ELEM_LOAD_BODY .AND. SIZE(val) >= 3) THEN
      CALL PH_Elem_T3D2_FormBodyForce(coords, val(1), val(2), val(3), F_eq)
    END IF
  END SUBROUTINE PH_Elem_T3D2_FormNodalForce

  !=============================================================================
  ! OUTPUT
  !=============================================================================
  SUBROUTINE PH_Elem_T3D2_CollectIPVars(ip_stress, ip_strain, ip_peeq, n_ip, out_vars)
  ...
  END SUBROUTINE PH_Elem_T3D2_CollectIPVars

  SUBROUTINE PH_Elem_T3D2_EvalAxialStress(strain, E_young, sigma)
    REAL(wp), INTENT(IN)  :: strain
    REAL(wp), INTENT(IN)  :: E_young
    REAL(wp), INTENT(OUT) :: sigma
    sigma = E_young * strain
  END SUBROUTINE PH_Elem_T3D2_EvalAxialStress

  SUBROUTINE PH_Elem_T3D2_GetExtrapMat(E)
    REAL(wp), INTENT(OUT) :: E(2, 1)
    E(1, 1) = ONE
    E(2, 1) = ONE
  END SUBROUTINE PH_Elem_T3D2_GetExtrapMat

  SUBROUTINE PH_Elem_T3D2_MapToNode(ip_vars, weights, node_vars)
    ...
    END IF
  END SUBROUTINE PH_Elem_T3D2_MapToNode

  SUBROUTINE PH_Elem_T3D2_Material_Update_Routed(rt_ctx, mat_slot, dstrain, &
    ...
    CALL PH_Elem_MatRoute_ElasticUniaxial(rt_ctx, mat_slot, dstrain, &
                                          stress_old, stress_new, D_tangent, status)
  END SUBROUTINE PH_Elem_T3D2_Material_Update_Routed

END MODULE PH_Elem_T3D2
