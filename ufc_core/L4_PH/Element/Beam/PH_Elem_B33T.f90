...
! DOMAIN: Element/Beam
! ROLE:   Proc
! BRIEF:  B33T 2D Euler-Bernoulli beam with thermal coupling
!===============================================================================
MODULE PH_Elem_B33T
  !===========================================================================
  ...
  ! L4_PH: Physics layer - reuse B33 mechanical kernel
  USE PH_Elem_B33, ONLY: PH_Elem_B33_FormStiffMatrixWithSection
  ...
  !===========================================================================
  INTEGER(i4), PARAMETER, PUBLIC :: PH_B33T_NDOF_TOTAL  = 8_i4   ! Total DOF (6 mech + 2 therm)
  INTEGER(i4), PARAMETER, PUBLIC :: PH_B33T_NDOF_MECH   = 6_i4   ! Mechanical DOF
  INTEGER(i4), PARAMETER, PUBLIC :: PH_B33T_NDOF_THERM  = 2_i4   ! Thermal DOF
  INTEGER(i4), PARAMETER, PUBLIC :: PH_B33T_NNODE       = 2_i4   ! Number of nodes
  
  ! Property indices (for props array access)
  INTEGER(i4), PARAMETER, PRIVATE :: PH_B33T_PROP_E     = 1_i4   ! Young's modulus index
  INTEGER(i4), PARAMETER, PRIVATE :: PH_B33T_PROP_NU    = 2_i4   ! Poisson's ratio index
  
  !===========================================================================
  ! Public API - Core computational subroutines
  !===========================================================================
  PUBLIC :: PH_Elem_B33T_FormStiffMatrix       ! Form total stiffness matrix (8x8)
  PUBLIC :: PH_Elem_B33T_FormIntForce          ! Form internal force vector (8x1)
  PUBLIC :: PH_Elem_B33T_ConsMassMatrix        ! Form consistent mass matrix (8x8)
  PUBLIC :: PH_Elem_B33T_LumpMassVector        ! Form lumped mass vector (8x1)
  PUBLIC :: UF_Elem_B33T_Calc                  ! Unified element calculation interface
...
CONTAINS
  ...
  !===========================================================================
  SUBROUTINE PH_Elem_B33T_FormStiffMatrix(coords3, E_young, nu, area, I_bend, &
       k_thermal, alpha_cte, Ke8, status)
    !-------------------------------------------------------------------------
    ! Purpose: Form total 8x8 stiffness matrix including:
    !   - Mechanical block (6x6) from B33 beam theory
    ...
    ! Form mechanical stiffness (6x6) using B33 kernel
    CALL PH_Elem_B33_FormStiffMatrixWithSection(coords3, E_young, nu, area, I_bend, Ke6)
    ...
    IF (ABS(alpha_cte) > 1.0e-30_wp .AND. el_len > 1.0e-20_wp .AND. &
      ...
      CALL PH_Elem_B33T_FillKutAxial(coords3, E_young, area, alpha_cte, el_len, Ke8)
    END IF
    
    status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_B33T_FormStiffMatrix

  !===========================================================================
  ! Internal Force Vector Formation
  !===========================================================================
  SUBROUTINE PH_Elem_B33T_FormIntForce(coords3, u8, E_young, nu, area, I_bend, &
    ...
    CALL PH_Elem_B33T_FormStiffMatrix(coords3, E_young, nu, area, I_bend, &
         k_thermal, alpha_cte, Ke8, status)
    IF (status%status_code /= IF_STATUS_OK) RETURN
    
    R8 = MATMUL(Ke8, u8)
    status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_B33T_FormIntForce
  
  !===========================================================================
  ! Consistent Mass Matrix Formation
  !===========================================================================
  SUBROUTINE PH_Elem_B33T_ConsMassMatrix(coords3, rho, area, Me8, status)
    !-------------------------------------------------------------------------
    ! Purpose: Form consistent mass matrix for B33T element
    ...
    ! Node 2 translational DOF (u_x=5, u_y=6)
    ...
    status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_B33T_ConsMassMatrix
  
  !===========================================================================
  ! Lumped Mass Vector Formation
  !===========================================================================
  SUBROUTINE PH_Elem_B33T_LumpMassVector(coords3, rho, area, M_lumped8, status)
    !-------------------------------------------------------------------------
    ...
    status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_B33T_LumpMassVector

  !===========================================================================
  ! Unified Element Calculation Interface (UFC L3->L4 Bridge)
  !===========================================================================
  SUBROUTINE UF_Elem_B33T_Calc(ElemType, Formul, Ctx, state_in, Mat, state_out, flags)
    !-------------------------------------------------------------------------
    ! Purpose: Unified element calculation interface for B33T
    ...
    ! Local variables
    REAL(wp) :: coords(3, 2)           ! Working coordinates (3D for B33 kernel)
    ...
    !-----------------------------------------------------------------------
    IF (ElemType%numNodes /= PH_B33T_NNODE .OR. ElemType%dim /= 2_i4) THEN
      flags%failed = .TRUE.
      flags%status%status_code = IF_STATUS_INVALID
      flags%status%message = 'UF_Elem_B33T_Calc: expect 2-node 2D beam (B33T)'
      RETURN
    END IF
    ...
    !-----------------------------------------------------------------------
    IF (.NOT. ALLOCATED(Ctx%coords_ref) .OR. SIZE(Ctx%coords_ref, 2) < PH_B33T_NNODE) THEN
      flags%failed = .TRUE.
      flags%status%status_code = IF_STATUS_INVALID
      flags%status%message = 'UF_Elem_B33T_Calc: coords_ref missing or insufficient'
      RETURN
    END IF

    !-----------------------------------------------------------------------
    ! Extract coordinates (embed 2D beam in 3D space for B33 kernel)
    !-----------------------------------------------------------------------
    coords(1:2, 1:2) = Ctx%coords_ref(1:2, 1:2)
    coords(3, 1:2)   = 0.0_wp  ! z-coordinate = 0 for plane beam

    !-----------------------------------------------------------------------
    ! Extract displacement vector (8 DOF: 6 mech + 2 therm)
    !-----------------------------------------------------------------------
    u8 = 0.0_wp
    IF (ALLOCATED(Ctx%disp_total) .AND. SIZE(Ctx%disp_total, 2) >= PH_B33T_NNODE) THEN
    ...
    IF (ALLOCATED(props%props)) THEN
      ! Elastic constants
      IF (SIZE(props%props) >= PH_B33T_PROP_E) &
        E_young = props%props(PH_B33T_PROP_E)
      IF (SIZE(props%props) >= PH_B33T_PROP_NU) &
        nu = props%props(PH_B33T_PROP_NU)
    ...
    IF (E_young <= 0.0_wp) THEN
      flags%failed = .TRUE.
      flags%status%status_code = IF_STATUS_INVALID
      flags%status%message = 'UF_Elem_B33T_Calc: invalid Young modulus (must be > 0)'
      RETURN
    END IF
    ...
    !-----------------------------------------------------------------------
    CALL PH_Elem_B33T_FormStiffMatrix(coords, E_young, nu, area_a, I_bend, &
    ...
    !-----------------------------------------------------------------------
    CALL PH_Elem_B33T_FormIntForce(coords, u8, E_young, nu, area_a, I_bend, &
    ...
    !-----------------------------------------------------------------------
    CALL PH_B33T_EnsureStorage8(state_out)
  ...
  END SUBROUTINE UF_Elem_B33T_Calc

  !===========================================================================
  ! Storage Management
  !===========================================================================
  SUBROUTINE PH_B33T_EnsureStorage8(state_out)
    !-------------------------------------------------------------------------
    ...
    ELSE IF (SIZE(state_out%evo%Ke, 1) /= PH_B33T_NDOF_TOTAL .OR. &
             SIZE(state_out%evo%Ke, 2) /= PH_B33T_NDOF_TOTAL) THEN
    ...
    IF (.NOT. ASSOCIATED(state_out%Re)) THEN
      ALLOCATE(state_out%Re(PH_B33T_NDOF_TOTAL))
    ELSE IF (SIZE(state_out%Re) /= PH_B33T_NDOF_TOTAL) THEN
      DEALLOCATE(state_out%Re)
      ALLOCATE(state_out%Re(PH_B33T_NDOF_TOTAL))
    END IF
  END SUBROUTINE PH_B33T_EnsureStorage8

  !===========================================================================
  ! Thermo-Mechanical Coupling Matrix (K_ut)
  !===========================================================================
  SUBROUTINE PH_Elem_B33T_FillKutAxial(coords3, E_young, area, alpha_cte, el_len, Ke8)
    !-------------------------------------------------------------------------
  ...
  END SUBROUTINE PH_Elem_B33T_FillKutAxial

END MODULE PH_Elem_B33T
!===============================================================================
! End of Module PH_ElemB33T_Algo
!
! Summary of Implementation (v1.0):
!   - Complete B33T thermo-mechanical coupling following B21T pattern
!   - 8 DOF system (6 mechanical + 2 thermal)
!   - Axial thermal expansion coupling K_ut/K_tu
!   - 1D thermal conduction between nodes
!   - UFC-compliant UF_Elem_B33T_Calc interface
!
! API Reference:
!   Core Functions:
!     - PH_Elem_B33T_FormStiffMatrix: Total 8x8 stiffness with coupling
!     - PH_Elem_B33T_FormIntForce: Internal force vector R = K*u
!     - PH_Elem_B33T_ConsMassMatrix: Consistent mass (translational only)
!     - PH_Elem_B33T_LumpMassVector: Lumped mass vector
!   
!   UFC Bridge:
!     - UF_Elem_B33T_Calc: Unified element calculation interface
!
! Related Modules:
!     - PH_ElemB21T_Algo: 2D plane beam thermal coupling template
!     - PH_ElemB33_Algo: Base mechanical kernel
!     - PH_ElemB31T_Algo: 3D beam thermal coupling
!===============================================================================
