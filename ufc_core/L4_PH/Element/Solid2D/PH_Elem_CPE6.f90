...
! ROLE:   Proc
! BRIEF:  CPE6 element definition (6-node plane strain triangle)
!===============================================================================
MODULE PH_Elem_CPE6
!> [CORE] CPE6 plane strain triangle (merged Defn+Sect+Constraints+Cont+Loads+Out)
  ...
  ! Constants
  INTEGER(i4), PARAMETER :: PH_ELEM_CPE6_NNODE  = 6_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_CPE6_NIP   = 3_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_CPE6_NDOF  = 12_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_CPE6_NEDGE = 3_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_CPE6_EDGE_NODES(2, 3) = RESHAPE([ 1,2, 2,3, 3,1 ], [2, 3])
  ! For CPE6P: face = edge in 2D triangle
  INTEGER(i4), PARAMETER :: PH_ELEM_CPE6_FACE_NODES(2, 3) = RESHAPE([ 1,2, 2,3, 3,1 ], [2, 3])
  ...
  ! PUBLIC
  !=============================================================================
  PUBLIC :: PH_Elem_CPE6_DefInit
  PUBLIC :: PH_Elem_CPE6_ShapeFunc_Arg
  PUBLIC :: PH_Elem_CPE6_ShapeFunc
  PUBLIC :: PH_Elem_CPE6_Jac_Arg
  PUBLIC :: PH_Elem_CPE6_Jac
  PUBLIC :: PH_Elem_CPE6_BMatrix_Arg
  PUBLIC :: PH_Elem_CPE6_BMatrix
  PUBLIC :: PH_Elem_CPE6_JacB_Arg
  PUBLIC :: PH_Elem_CPE6_JacB
  PUBLIC :: PH_Elem_CPE6_Strain_Arg
  PUBLIC :: PH_Elem_CPE6_Strain
  PUBLIC :: PH_Elem_CPE6_Stress_Arg
  PUBLIC :: PH_Elem_CPE6_Stress
  PUBLIC :: PH_Elem_CPE6_StiffMatrix_Arg
  PUBLIC :: PH_Elem_CPE6_FormStiffMatrix
  PUBLIC :: PH_Elem_CPE6_NL_TL_Arg
  PUBLIC :: PH_Elem_CPE6_NL_TL
  PUBLIC :: PH_Elem_CPE6_NL_UL_Arg
  PUBLIC :: PH_Elem_CPE6_NL_UL
  PUBLIC :: PH_Elem_CPE6_GaussPoints
  PUBLIC :: PH_Elem_CPE6_ConstMatrix
  PUBLIC :: PH_Elem_CPE6_FormIntForce
  PUBLIC :: PH_Elem_CPE6_FormIntForceFromStress
  PUBLIC :: PH_Elem_CPE6_ConsMass
  PUBLIC :: PH_Elem_CPE6_LumpMass
  PUBLIC :: PH_Elem_CPE6_ThermStrainVector
  PUBLIC :: PH_Elem_CPE6_GetArea
  PUBLIC :: PH_Elem_CPE6_GetCentroid
  PUBLIC :: PH_Elem_CPE6_GetSectProps
  PUBLIC :: PH_Elem_CPE6_ApplyConstraint
  PUBLIC :: PH_Elem_CPE6_ApplyMPC
  PUBLIC :: PH_Elem_CPE6_FormContactContrib
  PUBLIC :: PH_Elem_CPE6_FormContactEdgeCtr
  PUBLIC :: PH_Elem_CPE6_FormNodalForce
  PUBLIC :: PH_Elem_CPE6_FormBodyForce
  PUBLIC :: PH_Elem_CPE6_FormEdgePressure
  PUBLIC :: PH_Elem_CPE6_CollectIPVars
  PUBLIC :: PH_Elem_CPE6_MapToNode
  PUBLIC :: PH_Elem_CPE6_GetExtrapMat
  PUBLIC :: PH_Elem_CPE6_EvalVonMises
  PUBLIC :: PH_Elem_CPE6_EvalPrincStress
  PUBLIC :: PH_Elem_CPE6_EvalStressInvar
  PUBLIC :: PH_Elem_CPE6_Material_Update_Routed
  PUBLIC :: PH_ELEM_CPE6_NNODE, PH_ELEM_CPE6_NIP, PH_ELEM_CPE6_NDOF, PH_ELEM_CPE6_NEDGE
  PUBLIC :: PH_ELEM_CPE6_EDGE_NODES, PH_ELEM_CPE6_FACE_NODES

  ! Generic interfaces for legacy + structured
  INTERFACE PH_Elem_CPE6_FormStiffMatrix
    MODULE PROCEDURE PH_Elem_CPE6_FormStiffMatrix_InOut
    MODULE PROCEDURE PH_Elem_CPE6_FormStiffMatrix_Legacy
  END INTERFACE

  INTERFACE PH_Elem_CPE6_BMatrix
    MODULE PROCEDURE PH_Elem_CPE6_BMatrix_InOut
    MODULE PROCEDURE PH_Elem_CPE6_BMatrix_Legacy
  END INTERFACE

  INTERFACE PH_Elem_CPE6_ShapeFunc
    MODULE PROCEDURE PH_Elem_CPE6_ShapeFunc_Legacy
    MODULE PROCEDURE PH_Elem_CPE6_ShapeFunc_InOut
  END INTERFACE

  INTERFACE PH_Elem_CPE6_Jac
    MODULE PROCEDURE PH_Elem_CPE6_Jac_Legacy
    MODULE PROCEDURE PH_Elem_CPE6_Jac_InOut
  END INTERFACE

  INTERFACE PH_Elem_CPE6_JacB
    MODULE PROCEDURE PH_Elem_CPE6_JacB_Legacy
    MODULE PROCEDURE PH_Elem_CPE6_JacB_InOut
  END INTERFACE

  INTERFACE PH_Elem_CPE6_Strain
    MODULE PROCEDURE PH_Elem_CPE6_Strain_Legacy
    MODULE PROCEDURE PH_Elem_CPE6_Strain_InOut
  END INTERFACE

  INTERFACE PH_Elem_CPE6_Stress
    MODULE PROCEDURE PH_Elem_CPE6_Stress_Legacy
    MODULE PROCEDURE PH_Elem_CPE6_Stress_InOut
  END INTERFACE

  INTERFACE PH_Elem_CPE6_NL_TL
    MODULE PROCEDURE PH_Elem_CPE6_NL_TL_Structured
    MODULE PROCEDURE PH_Elem_CPE6_NL_TL_Legacy
  END INTERFACE

  INTERFACE PH_Elem_CPE6_NL_UL
    MODULE PROCEDURE PH_Elem_CPE6_NL_UL_Structured
    MODULE PROCEDURE PH_Elem_CPE6_NL_UL_Legacy
  END INTERFACE
...
CONTAINS
  ...
  !=============================================================================
  SUBROUTINE PH_Elem_CPE6_ShapeFunc_Legacy(xi, eta, N, dNdxi)
    REAL(wp), INTENT(IN)  :: xi, eta
    REAL(wp), INTENT(OUT) :: N(6)
    REAL(wp), INTENT(OUT) :: dNdxi(2, 6)
    TYPE(PH_Elem_CPE6_ShapeFunc_Arg) :: in
    TYPE(PH_Elem_CPE6_ShapeFunc_Arg) :: out
    in%xi = xi
    in%eta = eta
    CALL PH_Elem_CPE6_ShapeFunc_InOut(arg)
    N = out%N
    dNdxi = out%dNdxi
  END SUBROUTINE PH_Elem_CPE6_ShapeFunc_Legacy

  SUBROUTINE PH_Elem_CPE6_Jac_Legacy(dNdxi, coords, J, detJ)
    REAL(wp), INTENT(IN)  :: dNdxi(2, 6)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(OUT) :: J(2, 2)
    REAL(wp), INTENT(OUT) :: detJ
    TYPE(PH_Elem_CPE6_Jac_Arg) :: in
    TYPE(PH_Elem_CPE6_Jac_Arg) :: out
    in%dNdxi = dNdxi
    in%coords = coords
    CALL PH_Elem_CPE6_Jac_InOut(arg)
    J = out%J
    detJ = out%detJ
  END SUBROUTINE PH_Elem_CPE6_Jac_Legacy

  SUBROUTINE PH_Elem_CPE6_JacB_Legacy(coords, xi_pt, eta_pt, N, dNdx, J, detJ, B)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    ...
    TYPE(PH_Elem_CPE6_JacB_Arg) :: in
    TYPE(PH_Elem_CPE6_JacB_Arg) :: out
    in%coords = coords
    in%xi = xi_pt
    in%eta = eta_pt
    CALL PH_Elem_CPE6_JacB_InOut(arg)
    N = out%N
    dNdx = out%dNdx
    J = out%J
    detJ = out%detJ
    B = out%B
  END SUBROUTINE PH_Elem_CPE6_JacB_Legacy

  SUBROUTINE PH_Elem_CPE6_Strain_Legacy(B, u, strain)
    REAL(wp), INTENT(IN)  :: B(3, 12)
    REAL(wp), INTENT(IN)  :: u(12)
    REAL(wp), INTENT(OUT) :: strain(3)
    TYPE(PH_Elem_CPE6_Strain_Arg) :: in
    TYPE(PH_Elem_CPE6_Strain_Arg) :: out
    in%B = B
    in%u = u
    CALL PH_Elem_CPE6_Strain_InOut(arg)
    strain = out%strain
  END SUBROUTINE PH_Elem_CPE6_Strain_Legacy

  SUBROUTINE PH_Elem_CPE6_Stress_Legacy(epsilon, D, sigma)
    REAL(wp), INTENT(IN)  :: epsilon(3)
    REAL(wp), INTENT(IN)  :: D(3, 3)
    REAL(wp), INTENT(OUT) :: sigma(3)
    TYPE(PH_Elem_CPE6_Stress_Arg) :: in
    TYPE(PH_Elem_CPE6_Stress_Arg) :: out
    in%epsilon = epsilon
    in%D = D
    CALL PH_Elem_CPE6_Stress_InOut(arg)
    sigma = out%sigma
  END SUBROUTINE PH_Elem_CPE6_Stress_Legacy

  !=============================================================================
  ! DEFINITION (core)
  !=============================================================================
  SUBROUTINE PH_ELEM_CPE6_AreaInt(coords, area)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    ...
    DO ip = 1, 3
      CALL PH_Elem_CPE6_ShapeFunc_Legacy(xi(ip), eta(ip), N, dNdxi)
      CALL PH_Elem_CPE6_Jac_Legacy(dNdxi, coords, J, detJ)
      area = area + detJ * weights(ip)
    END DO
  END SUBROUTINE PH_ELEM_CPE6_AreaInt

  SUBROUTINE PH_Elem_CPE6_ThermStrainVector(alpha, deltaT, eps_th)
    ...
    eps_th(2) = e
    eps_th(3) = ZERO
  END SUBROUTINE PH_Elem_CPE6_ThermStrainVector

  SUBROUTINE PH_Elem_CPE6_BMatrix_Legacy(dNdx, B)
    REAL(wp), INTENT(IN)  :: dNdx(2, 6)
    REAL(wp), INTENT(OUT) :: B(3, 12)
    TYPE(PH_Elem_CPE6_BMatrix_Arg) :: in
    TYPE(PH_Elem_CPE6_BMatrix_Arg) :: out
    in%dNdx = dNdx
    CALL PH_Elem_CPE6_BMatrix_InOut(arg)
    B = out%B
  END SUBROUTINE PH_Elem_CPE6_BMatrix_Legacy

  SUBROUTINE PH_Elem_CPE6_BMatrix_InOut(arg)
    TYPE(PH_Elem_CPE6_BMatrix_Arg), INTENT(INOUT) :: arg
    INTEGER(i4) :: i
  ...
  END SUBROUTINE PH_Elem_CPE6_BMatrix_InOut

  SUBROUTINE PH_Elem_CPE6_ConsMass(coords, rho, Me)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    ...
    DO ip = 1, 3
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_CPE6_ConsMass

  SUBROUTINE PH_Elem_CPE6_ConstMatrix(E_young, nu, D)
  ...
  END SUBROUTINE PH_Elem_CPE6_ConstMatrix

  SUBROUTINE PH_Elem_CPE6_DefInit()
  END SUBROUTINE PH_Elem_CPE6_DefInit

  SUBROUTINE PH_Elem_CPE6_FormIntForce(coords, u, E_young, nu, R_int)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(IN)  :: u(12)
    REAL(wp), INTENT(IN)  :: E_young, nu
    ...
    INTEGER(i4) :: ip
    R_int = ZERO
    CALL PH_Elem_CPE6_ConstMatrix(E_young, nu, D)
    CALL PH_Elem_CPE6_GaussPoints(xi, eta, weights)
    DO ip = 1, 3
      CALL PH_Elem_CPE6_JacB_Legacy(coords, xi(ip), eta(ip), N, dNdx, J, detJ, B)
      IF (ABS(detJ) <= 1.0e-12_wp) CYCLE
      CALL PH_Elem_CPE6_Strain_Legacy(B, u, strain)
      CALL PH_Elem_CPE6_Stress_Legacy(strain, D, sigma)
      R_int = R_int + MATMUL(TRANSPOSE(B), sigma) * detJ * weights(ip)
    END DO
  END SUBROUTINE PH_Elem_CPE6_FormIntForce

  SUBROUTINE PH_Elem_CPE6_FormIntForceFromStress(coords, sigma3, R_int)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    ...
    END DO
  END SUBROUTINE PH_Elem_CPE6_FormIntForceFromStress

  SUBROUTINE PH_Elem_CPE6_FormStiffMatrix_Legacy(coords, E_young, nu, Ke)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(IN)  :: E_young, nu
    REAL(wp), INTENT(OUT) :: Ke(12, 12)
    TYPE(PH_Elem_CPE6_StiffMatrix_Arg) :: sm
    REAL(wp) :: D(3, 3)
    CALL PH_Elem_CPE6_ConstMatrix(E_young, nu, D)
    sm%coords = coords
    sm%D_matrix = D
    CALL PH_Elem_CPE6_FormStiffMatrix_InOut(sm)
    Ke = sm%evo%Ke
  END SUBROUTINE PH_Elem_CPE6_FormStiffMatrix_Legacy

  SUBROUTINE PH_Elem_CPE6_FormStiffMatrix_InOut(arg)
    TYPE(PH_Elem_CPE6_StiffMatrix_Arg), INTENT(INOUT) :: arg
    REAL(wp) :: xi(3), eta(3), weights(3)
    TYPE(PH_Elem_CPE6_JacB_Arg) :: jb
    REAL(wp) :: BTD(12, 3), dV
    INTEGER(i4) :: ip
    CALL init_error_status(arg%status)
    arg%evo%Ke = ZERO
    ...
    DO ip = 1, 3
      jb%xi = xi(ip)
      jb%eta = eta(ip)
      CALL PH_Elem_CPE6_JacB_InOut(jb)
    ...
    END DO
    arg%status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_CPE6_FormStiffMatrix

  SUBROUTINE PH_Elem_CPE6_GaussPoints(xi, eta, weights)
    ...
    eta(3) = s23
    weights(3) = w6
  END SUBROUTINE PH_Elem_CPE6_GaussPoints

  SUBROUTINE PH_Elem_CPE6_Jac_InOut(arg)
    TYPE(PH_Elem_CPE6_Jac_Arg), INTENT(INOUT) :: arg
    INTEGER(i4) :: i
    CALL init_error_status(arg%status)
    arg%J = ZERO
    DO i = 1, 6
    ...
    arg%status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_CPE6_Jac_InOut

  SUBROUTINE PH_Elem_CPE6_JacB_InOut(arg)
    TYPE(PH_Elem_CPE6_JacB_Arg), INTENT(INOUT) :: arg
    TYPE(PH_Elem_CPE6_ShapeFunc_Arg) :: sf
    TYPE(PH_Elem_CPE6_Jac_Arg) :: jac
    TYPE(PH_Elem_CPE6_BMatrix_Arg) :: bm
    REAL(wp) :: dNdxi(2, 6), Jinv(2, 2)
    INTEGER(i4) :: i
    CALL init_error_status(arg%status)
    sf%xi = arg%xi
    sf%eta = arg%eta
    CALL PH_Elem_CPE6_ShapeFunc_InOut(sf)
    ...
    CALL PH_Elem_CPE6_Jac_InOut(jac)
    ...
    IF (ABS(arg%detJ) > 1.0e-20_wp) THEN
      ...
      END DO
      bm%dNdx = arg%dNdx
      CALL PH_Elem_CPE6_BMatrix_InOut(bm)
    ...
    END IF
    arg%status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_CPE6_JacB_InOut

  SUBROUTINE PH_Elem_CPE6_LumpMass(coords, rho, M_lumped)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(IN)  :: rho
    REAL(wp), INTENT(OUT) :: M_lumped(12)
    REAL(wp) :: area, m
    INTEGER(i4) :: i
    CALL PH_ELEM_CPE6_AreaInt(coords, area)
    m = rho * area / 6.0_wp
    DO i = 1, 6
      M_lumped(2*i-1) = m
      M_lumped(2*i)   = m
    END DO
  END SUBROUTINE PH_Elem_CPE6_LumpMass

  SUBROUTINE PH_Elem_CPE6_ShapeFunc_InOut(arg)
    TYPE(PH_Elem_CPE6_ShapeFunc_Arg), INTENT(INOUT) :: arg
    ...
    arg%status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_CPE6_ShapeFunc_InOut

  SUBROUTINE PH_Elem_CPE6_Strain_InOut(arg)
    TYPE(PH_Elem_CPE6_Strain_Arg), INTENT(INOUT) :: arg
    INTEGER(i4) :: j
    CALL init_error_status(arg%status)
    arg%strain = MATMUL(arg%B, arg%u)
    arg%status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_CPE6_Strain_InOut

  SUBROUTINE PH_Elem_CPE6_Stress_InOut(arg)
    TYPE(PH_Elem_CPE6_Stress_Arg), INTENT(INOUT) :: arg
    INTEGER(i4) :: j
    CALL init_error_status(arg%status)
    arg%sigma = ZERO
    DO j = 1, 3
    ...
    END DO
    arg%status%status_code = IF_STATUS_OK
  END SUBROUTINE PH_Elem_CPE6_Stress_InOut

  !=============================================================================
  ! NL_TL / NL_UL - Legacy (for RT_AsmNLGeomDispatch) and Structured
  !=============================================================================
  SUBROUTINE PH_Elem_CPE6_NL_TL_Legacy(coords_ref, u_elem, D, Ke_mat, Ke_geo, R_int, status)
    ...
    DO igp = 1, 3
      CALL PH_Elem_CPE6_ShapeFunc_Legacy(xi_gp(igp), eta_gp(igp), N, dN_dxi)
      J_ref = ZERO
      DO i = 1, 6
  ...
  END SUBROUTINE PH_Elem_CPE6_NL_TL_Legacy

  SUBROUTINE PH_Elem_CPE6_NL_UL_Legacy(coords_prev, u_incr, D, Ke_mat, Ke_geo, R_int, status)
  ...
  END SUBROUTINE PH_Elem_CPE6_NL_UL_Legacy

  ! Structured NL_TL/NL_UL (delegate to legacy with D from mat_prop)
  SUBROUTINE PH_Elem_CPE6_NL_TL_Structured(arg)
    TYPE(PH_Elem_CPE6_NL_TL_Arg), INTENT(INOUT) :: arg
    ...
    IF (ALLOCATED(arg%mat_prop%props) .AND. SIZE(arg%mat_prop%props) >= 2) THEN
      ...
      CALL PH_Elem_CPE6_ConstMatrix(E_young, nu, D)
    END IF
    CALL PH_Elem_CPE6_NL_TL_Legacy(arg%coords_ref, arg%lcl%u_elem, D, &
      arg%evo%Ke_mat, arg%evo%Ke_geo, arg%evo%R_int, arg%status)
    IF (ALLOCATED(arg%mat_state)) arg%mat_state = arg%mat_state
  END SUBROUTINE PH_Elem_CPE6_NL_TL_Structured

  SUBROUTINE PH_Elem_CPE6_NL_UL_Structured(arg)
    TYPE(PH_Elem_CPE6_NL_UL_Arg), INTENT(INOUT) :: arg
    REAL(wp) :: D(3, 3), E_young, nu
    D = ZERO
    D(1, 1) = ONE
    D(2, 2) = ONE
    ...
    IF (ALLOCATED(arg%mat_prop%props) .AND. SIZE(arg%mat_prop%props) >= 2) THEN
      ...
      CALL PH_Elem_CPE6_ConstMatrix(E_young, nu, D)
    END IF
    CALL PH_Elem_CPE6_NL_UL_Legacy(arg%coords_prev, arg%u_incr, D, &
      arg%evo%Ke_mat, arg%evo%Ke_geo, arg%evo%R_int, arg%status)
    IF (ALLOCATED(arg%mat_state)) arg%mat_state = arg%mat_state
  END SUBROUTINE PH_Elem_CPE6_NL_UL_Structured

  !=============================================================================
  ! SECTION (inlined)
  !=============================================================================
  SUBROUTINE PH_Elem_CPE6_GetArea(coords, area)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(OUT) :: area
    CALL PH_ELEM_CPE6_AreaInt(coords, area)
  END SUBROUTINE PH_Elem_CPE6_GetArea

  SUBROUTINE PH_Elem_CPE6_GetCentroid(coords, centroid)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    ...
    END IF
  END SUBROUTINE PH_Elem_CPE6_GetCentroid

  SUBROUTINE PH_Elem_CPE6_GetSectProps(coords, density_in, area, mass)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(IN)  :: density_in
    REAL(wp), INTENT(OUT) :: area
    REAL(wp), INTENT(OUT) :: mass
    CALL PH_Elem_CPE6_GetArea(coords, area)
    mass = density_in * area
  END SUBROUTINE PH_Elem_CPE6_GetSectProps

  !=============================================================================
  ! CONSTRAINTS (inlined)
  !=============================================================================
  SUBROUTINE PH_Elem_CPE6_ApplyConstraint(ctype, idof, val, penalty, K_el, F_el)
  ...
  END SUBROUTINE PH_Elem_CPE6_ApplyConstraint

  SUBROUTINE PH_Elem_CPE6_ApplyMPC(c, val, penalty, K_el, F_el)
    ...
    DO i = 1, 12
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_CPE6_ApplyMPC

  !=============================================================================
  ! CONTACT (inlined)
  !=============================================================================
  SUBROUTINE PH_Elem_CPE6_FormContactContrib(edge_id, xi, eta, N, n, gap, penalty, edge_len, K_el, F_el)
    ...
    DO a = 1, 6
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_CPE6_FormContactContrib

  SUBROUTINE PH_Elem_CPE6_FormContactEdgeCtr(edge_id, coords, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: edge_id
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(IN)  :: gap
    REAL(wp), INTENT(IN)  :: penalty
    ...
    CASE (3)
      ...
      eta = HALF
    END SELECT
    CALL PH_Elem_CPE6_ShapeFunc_Legacy(xi, eta, N, dNdxi)
    ...
    n(1) = -t(2) / len
    n(2) =  t(1) / len
    CALL PH_Elem_CPE6_FormContactContrib(edge_id, xi, eta, N, n, gap, penalty, len, K_el, F_el)
  END SUBROUTINE PH_Elem_CPE6_FormContactEdgeCtr

  !=============================================================================
  ! LOADS (inlined)
  !=============================================================================
  SUBROUTINE PH_Elem_CPE6_FormEdgePressure(coords, p, edge_id, F_eq)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    REAL(wp), INTENT(IN)  :: p
    INTEGER(i4), INTENT(IN)  :: edge_id
    ...
    F_eq(ia)   = F_eq(ia)   + f3 * nx
    F_eq(ia+1) = F_eq(ia+1) + f3 * ny
  END SUBROUTINE PH_Elem_CPE6_FormEdgePressure

  SUBROUTINE PH_Elem_CPE6_FormBodyForce(coords, bx, by, F_eq)
    REAL(wp), INTENT(IN)  :: coords(2, 6)
    ...
    DO ip = 1, 3
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_CPE6_FormBodyForce

  SUBROUTINE PH_Elem_CPE6_FormNodalForce(load_type, coords, val, edge_id, F_eq)
    ...
    IF (load_type == PH_ELEM_LOAD_BODY) THEN
      CALL PH_Elem_CPE6_FormBodyForce(coords, val(1), val(2), F_eq)
    ELSE IF (load_type == PH_ELEM_LOAD_EDGE_P .AND. SIZE(val) >= 1) THEN
      CALL PH_Elem_CPE6_FormEdgePressure(coords, val(1), edge_id, F_eq)
    END IF
  END SUBROUTINE PH_Elem_CPE6_FormNodalForce
  ...
  SUBROUTINE PH_Elem_CPE6_CollectIPVars(ip_stress, ip_strain, ip_peeq, n_ip, out_vars)
    ...
    DO ip = 1, MIN(n_ip, 3)
      ...
      END IF
    END DO
  END SUBROUTINE PH_Elem_CPE6_CollectIPVars

  SUBROUTINE PH_Elem_CPE6_EvalPrincStress(sigma, principal)
    ...
    principal(1) = p + q
    principal(2) = p - q
  END SUBROUTINE PH_Elem_CPE6_EvalPrincStress

  SUBROUTINE PH_Elem_CPE6_EvalStressInvar(sigma, I1, J2)
    REAL(wp), INTENT(IN)  :: sigma(3)
  ...
  END SUBROUTINE PH_Elem_CPE6_EvalStressInvar

  SUBROUTINE PH_Elem_CPE6_EvalVonMises(sigma, seq)
    REAL(wp), INTENT(IN)  :: sigma(3)
  ...
  END SUBROUTINE PH_Elem_CPE6_EvalVonMises

  SUBROUTINE PH_Elem_CPE6_GetExtrapMat(E)
    ...
    E(5, :) = HALF * (E(2, :) + E(3, :))
    E(6, :) = HALF * (E(3, :) + E(1, :))
  END SUBROUTINE PH_Elem_CPE6_GetExtrapMat

  SUBROUTINE PH_Elem_CPE6_MapToNode(ip_vars, weights, node_vars)
    ...
    node_vars = ZERO
    CALL PH_Elem_CPE6_GetExtrapMat(E)
    ...
    DO ic = 1, n_comp
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_CPE6_MapToNode

  SUBROUTINE PH_Elem_CPE6_Material_Update_Routed(rt_ctx, mat_slot, dStrain, &
    ...
    CALL PH_Elem_MatRoute_ElasticPlaneStrain(rt_ctx, mat_slot, dStrain, &
                                             stress_old, stress_new, D_tangent, status)
  END SUBROUTINE PH_Elem_CPE6_Material_Update_Routed

END MODULE PH_Elem_CPE6
