!===============================================================================
! MODULE: PH_Elem_ShapeMechField
! LAYER:  L4_PH
! DOMAIN: Element
! ROLE:   Proc
! BRIEF:  Unified N, dNdx, B (strain-displacement), detJ for Piezo coupling
!===============================================================================
MODULE PH_Elem_ShapeMechField
  ...
  PUBLIC :: PH_ShapeMechanicalField_GetNumGauss
  PUBLIC :: PH_ShapeMechanicalField_Eval
  PUBLIC :: PH_ShapeMechanicalField_Supported
...
CONTAINS
  ...
  !-----------------------------------------------------------------------------
  PURE FUNCTION PH_ShapeMechanicalField_GetNumGauss(elem_type_id, npe) RESULT(n_ip)
    ...
    CASE DEFAULT
      n_ip = 0_i4
    END SELECT
  END FUNCTION PH_ShapeMechanicalField_GetNumGauss

  !-----------------------------------------------------------------------------
  ! Check if elem_type is supported for mechanical (B-matrix) assembly.
  !-----------------------------------------------------------------------------
  PURE FUNCTION PH_ShapeMechanicalField_Supported(elem_type_id, npe) RESULT(ok)
    INTEGER(i4), INTENT(IN) :: elem_type_id, npe
    LOGICAL :: ok
    ok = (RT_Asm_ShapeMechanicalField_GetNumGauss(elem_type_id, npe) > 0_i4)
  END FUNCTION PH_ShapeMechanicalField_Supported

  !-----------------------------------------------------------------------------
  ! Eval: Get N, dNdx, B (strain-displacement), detJ, weight at Gauss point ip.
  !   B(6, 3*npe) for 3D; B_phi = dNdx.
  !-----------------------------------------------------------------------------
  SUBROUTINE PH_ShapeMechanicalField_Eval(elem_type_id, coords, npe, ip, N, dNdx, B_u, detJ, weight, status)
  ...
  END SUBROUTINE PH_ShapeMechanicalField_Eval

END MODULE PH_Elem_ShapeMechField