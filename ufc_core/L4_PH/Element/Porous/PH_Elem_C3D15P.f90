!===============================================================================
! MODULE: PH_Elem_C3D15P
! LAYER:  L4_PH
! DOMAIN: Element/Porous
! ROLE:   Proc
! BRIEF:  C3D15P 15-node quadratic wedge with pore pressure
!===============================================================================
MODULE PH_Elem_C3D15P
!> [CORE] C3D15P element unified interface (merged 6 files)
  ...
  USE PH_Elem_C3D15, ONLY: &
    ...
    PH_Elem_C3D15_GaussPoints, PH_Elem_C3D15_JacB, PH_ELEM_C3D15_FACE_NODES
  IMPLICIT NONE
  PRIVATE

  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NNODE = 15_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NIP   = 9_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NDPN_STRUCT = 3_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NDPN_PORE   = 1_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NDPN_TOTAL  = 4_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NDOF_STRUCT = 45_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NDOF_PORE   = 15_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_C3D15P_NDOF        = 60_i4
  ...
  INTEGER(i4), PARAMETER :: PH_ELEM_N_IP = 9, PH_ELEM_N_NODE = 15

  PUBLIC :: PH_Elem_C3D15P_DefInit, PH_Elem_C3D15P_ShapeFunc, PH_Elem_C3D15P_Jac
  PUBLIC :: PH_Elem_C3D15P_BMatrix, PH_Elem_C3D15P_BpMatrix, PH_Elem_C3D15P_GaussPoints
  PUBLIC :: PH_Elem_C3D15P_JacB, PH_Elem_C3D15P_FormStiffMatrix, PH_Elem_C3D15P_FormIntForce
  PUBLIC :: PH_ELEM_C3D15P_NNODE, PH_ELEM_C3D15P_NIP, PH_ELEM_C3D15P_NDOF, PH_ELEM_C3D15P_NDOF_STRUCT, PH_ELEM_C3D15P_NDOF_PORE
  PUBLIC :: PH_ELEM_C3D15P_NDPN_STRUCT, PH_ELEM_C3D15P_NDPN_PORE, PH_ELEM_C3D15P_NDPN_TOTAL
  PUBLIC :: PH_Elem_C3D15P_NL_TL, PH_Elem_C3D15P_NL_UL
  PUBLIC :: PH_Elem_C3D15P_GetVolume, PH_Elem_C3D15P_GetSectProps
  PUBLIC :: PH_Elem_C3D15P_GetCentroid, PH_Elem_C3D15P_GetInertiaOrig
  PUBLIC :: PH_Elem_C3D15P_ApplyConstraint, PH_Elem_C3D15P_ApplyMPC
  PUBLIC :: PH_ELEM_CTYPE_PENALTY_DOF, PH_ELEM_CTYPE_MPC_LINEAR
  PUBLIC :: PH_Elem_C3D15P_FormContactContrib, PH_Elem_C3D15P_FormContactFaceCtr
  PUBLIC :: PH_Elem_C3D15P_FormNodalForce, PH_Elem_C3D15P_FormBodyForce
  PUBLIC :: PH_Elem_C3D15P_FormFacePressure, PH_Elem_C3D15P_FormPoreSource
  PUBLIC :: PH_ELEM_LOAD_BODY, PH_ELEM_LOAD_FACE_P, PH_ELEM_LOAD_PORE_SOURCE
  PUBLIC :: PH_Elem_C3D15P_CollectIPVars, PH_Elem_C3D15P_MapToNode
  PUBLIC :: PH_Elem_C3D15P_GetExtrapMat, PH_Elem_C3D15P_EvalVonMises
  PUBLIC :: PH_Elem_C3D15P_EvalPrincStress, PH_Elem_C3D15P_EvalStressInvar
...
CONTAINS
  ...
  SUBROUTINE PH_Elem_C3D15P_BpMatrix(dNdx, Bp)
    REAL(wp), INTENT(IN)  :: dNdx(3, 15)
    REAL(wp), INTENT(OUT) :: Bp(3, 15)
    INTEGER(i4) :: i
    DO i = 1, 15
      Bp(1, i) = dNdx(1, i)
      Bp(2, i) = dNdx(2, i)
      Bp(3, i) = dNdx(3, i)
    END DO
  END SUBROUTINE PH_Elem_C3D15P_BpMatrix

  SUBROUTINE PH_Elem_C3D15P_BMatrix(dNdx, B)
    REAL(wp), INTENT(IN)  :: dNdx(3, 15)
    REAL(wp), INTENT(OUT) :: B(6, 45)
    CALL PH_Elem_C3D15_BMatrix(dNdx, B)
  END SUBROUTINE PH_Elem_C3D15P_BMatrix

  SUBROUTINE PH_Elem_C3D15P_DefInit()
  END SUBROUTINE PH_Elem_C3D15P_DefInit

  SUBROUTINE PH_Elem_C3D15P_GaussPoints(xi, eta, zeta, weights)
    REAL(wp), INTENT(OUT) :: xi(9), eta(9), zeta(9), weights(9)
    CALL PH_Elem_C3D15_GaussPoints(xi, eta, zeta, weights)
  END SUBROUTINE PH_Elem_C3D15P_GaussPoints

  SUBROUTINE PH_Elem_C3D15P_ShapeFunc(xi, eta, zeta, N, dNdxi)
  ...
  END SUBROUTINE PH_Elem_C3D15P_ShapeFunc

  SUBROUTINE PH_Elem_C3D15P_Jac(dNdxi, coords, J, detJ)
  ...
  END SUBROUTINE PH_Elem_C3D15P_Jac

  SUBROUTINE PH_Elem_C3D15P_JacB(coords, xi, eta, zeta, N, dNdx, J, detJ, B, Bp)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    REAL(wp), INTENT(IN)  :: xi, eta, zeta
    REAL(wp), INTENT(OUT) :: N(15)
    REAL(wp), INTENT(OUT) :: dNdx(3, 15)
    REAL(wp), INTENT(OUT) :: J(3, 3), detJ
    REAL(wp), INTENT(OUT) :: B(6, 45), Bp(3, 15)
    CALL PH_Elem_C3D15_JacB(coords, xi, eta, zeta, N, dNdx, J, detJ, B)
    CALL PH_Elem_C3D15P_BpMatrix(dNdx, Bp)
  END SUBROUTINE PH_Elem_C3D15P_JacB

  SUBROUTINE PH_Elem_C3D15P_FormStiffMatrix(coords, D_struct, k_hyd, alpha_b, Ke)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    ...
    REAL(wp) :: N(15), dNdx(3, 15), J(3, 3), detJ, B(6, 45), Bp(3, 15)
    REAL(wp) :: m_vec(6), dV
    REAL(wp) :: Kuu_block(45, 45), Kpp_block(15, 15)
    REAL(wp) :: Kup_block(45, 15)
    INTEGER(i4) :: ip, i, j
    Ke = ZERO
    m_vec = ZERO
    m_vec(1:3) = ONE
    CALL PH_Elem_C3D15P_GaussPoints(xi, eta, zeta, weights)
    DO ip = 1, 9
      CALL PH_Elem_C3D15P_JacB(coords, xi(ip), eta(ip), zeta(ip), N, dNdx, J, detJ, B, Bp)
      IF (ABS(detJ) <= 1.0e-12_wp) CYCLE
      dV = detJ * weights(ip)
      Kuu_block = MATMUL(MATMUL(TRANSPOSE(B), D_struct), B) * dV
      Ke(1:45, 1:45) = Ke(1:45, 1:45) + Kuu_block
      Kpp_block = k_hyd * MATMUL(TRANSPOSE(Bp), Bp) * dV
      Ke(46:60, 46:60) = Ke(46:60, 46:60) + Kpp_block
      DO i = 1, 45
        DO j = 1, 15
          Kup_block(i, j) = alpha_b * DOT_PRODUCT(B(:, i), m_vec) * N(j) * dV
          Ke(i, 45+j) = Ke(i, 45+j) + Kup_block(i, j)
        END DO
      END DO
      DO i = 1, 15
        DO j = 1, 45
          Ke(45+i, j) = Ke(45+i, j) + Kup_block(j, i)
        END DO
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D15P_FormStiffMatrix

  SUBROUTINE PH_Elem_C3D15P_FormIntForce(coords, u_struct, p_pore, D_struct, k_hyd, alpha_b, R_int)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    REAL(wp), INTENT(IN)  :: u_struct(45), p_pore(15)
    REAL(wp), INTENT(IN)  :: D_struct(6, 6)
    REAL(wp), INTENT(IN)  :: k_hyd, alpha_b
    ...
    DO ip = 1, 9
      ...
      R_int(46:60) = R_int(46:60) + k_hyd * MATMUL(TRANSPOSE(Bp), grad_p) * dV
    END DO
  END SUBROUTINE PH_Elem_C3D15P_FormIntForce

  SUBROUTINE PH_Elem_C3D15P_NL_TL(coords_ref, u_elem, D, k_hyd, alpha_b, &
    ...
    CALL PH_Elem_C3D15P_FormStiffMatrix(coords_ref, D, k_hyd, alpha_b, Ke_mat)
    CALL PH_Elem_C3D15P_FormIntForce(coords_ref, u_elem(1:45), u_elem(46:60), &
                                    D, k_hyd, alpha_b, R_int)
  END SUBROUTINE PH_Elem_C3D15P_NL_TL

  SUBROUTINE PH_Elem_C3D15P_NL_UL(coords_prev, u_incr, D, k_hyd, alpha_b, &
                                  Ke_mat, Ke_geo, R_int, status)
    REAL(wp), INTENT(IN)  :: coords_prev(3, 15)
    REAL(wp), INTENT(IN)  :: u_incr(60)
    REAL(wp), INTENT(IN)  :: D(6, 6)
    REAL(wp), INTENT(IN)  :: k_hyd, alpha_b
    REAL(wp), INTENT(OUT) :: Ke_mat(60, 60), Ke_geo(60, 60), R_int(60)
    TYPE(ErrorStatusType), INTENT(OUT) :: status
    ...
    R_int  = ZERO
    status%code = STATUS_SUCCESS
    CALL PH_Elem_C3D15P_FormStiffMatrix(coords_prev, D, k_hyd, alpha_b, Ke_mat)
    CALL PH_Elem_C3D15P_FormIntForce(coords_prev, u_incr(1:45), u_incr(46:60), &
                                    D, k_hyd, alpha_b, R_int)
  END SUBROUTINE PH_Elem_C3D15P_NL_UL

  SUBROUTINE PH_Elem_C3D15P_GetCentroid(coords, centroid)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    ...
    CALL PH_Elem_C3D15P_GaussPoints(xi, eta, zeta, weights)
    DO ip = 1, 9
      CALL PH_Elem_C3D15P_ShapeFunc(xi(ip), eta(ip), zeta(ip), N, dNdxi)
      CALL PH_Elem_C3D15P_Jac(dNdxi, coords, J, detJ)
      dV = detJ * weights(ip)
      volume = volume + dV
      DO i = 1, 3
        DO j = 1, 15
  ...
  END SUBROUTINE PH_Elem_C3D15P_GetCentroid

  SUBROUTINE PH_Elem_C3D15P_GetInertiaOrig(coords, rho, I_out)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    ...
    DO ip = 1, 9
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D15P_GetInertiaOrig

  SUBROUTINE PH_Elem_C3D15P_GetSectProps(coords, density_in, volume, mass)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    REAL(wp), INTENT(IN)  :: density_in
    REAL(wp), INTENT(OUT) :: volume, mass
    CALL PH_Elem_C3D15P_GetVolume(coords, volume)
    mass = density_in * volume
  END SUBROUTINE PH_Elem_C3D15P_GetSectProps

  SUBROUTINE PH_Elem_C3D15P_GetVolume(coords, volume)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D15P_GetVolume

  SUBROUTINE PH_Elem_C3D15P_ApplyConstraint(ctype, idof, val, penalty, K_el, F_el)
    ...
    IF (idof < 1 .OR. idof > PH_ELEM_C3D15P_NDOF) RETURN
    K_el(idof, idof) = K_el(idof, idof) + penalty
    F_el(idof) = F_el(idof) + penalty * val
  END SUBROUTINE PH_Elem_C3D15P_ApplyConstraint

  SUBROUTINE PH_Elem_C3D15P_ApplyMPC(c, val, penalty, K_el, F_el)
    ...
    REAL(wp), INTENT(INOUT) :: F_el(60)
    INTEGER(i4) :: i, j
    DO i = 1, PH_ELEM_C3D15P_NDOF
      F_el(i) = F_el(i) + penalty * val * c(i)
      DO j = 1, PH_ELEM_C3D15P_NDOF
        K_el(i, j) = K_el(i, j) + penalty * c(i) * c(j)
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D15P_ApplyMPC

  SUBROUTINE PH_Elem_C3D15P_FormContactContrib(face_id, xi, eta, zeta, N, nodes, nnode_face, n, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: face_id
    REAL(wp), INTENT(IN)  :: xi, eta, zeta
    REAL(wp), INTENT(IN)  :: N(4)
    INTEGER(i4), INTENT(IN)  :: nodes(4)
    INTEGER(i4), INTENT(IN)  :: nnode_face
    ...
    DO a = 1, nnode_face
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D15P_FormContactContrib

  SUBROUTINE PH_Elem_C3D15P_FormContactFaceCtr(face_id, coords, gap, penalty, K_el, F_el)
    INTEGER(i4), INTENT(IN)  :: face_id
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    REAL(wp), INTENT(IN)  :: gap
    REAL(wp), INTENT(IN)  :: penalty
    REAL(wp), INTENT(OUT) :: K_el(60, 60)
    REAL(wp), INTENT(OUT) :: F_el(60)
    REAL(wp) :: xi, eta, zeta, N(15), N_face(4), n(3), dNdxi(3, 15)
    REAL(wp) :: r1(3), r2(3)
    INTEGER(i4) :: nodes(4), nnode_face, j
    K_el = ZERO
    F_el = ZERO
    ...
    ELSE
      ...
      zeta = 0.0_wp
    END IF
    CALL PH_Elem_C3D15P_ShapeFunc(xi, eta, zeta, N, dNdxi)
    DO j = 1, nnode_face
      N_face(j) = N(nodes(j))
    END DO
    ...
    CALL PH_Elem_C3D15P_FormContactContrib(face_id, xi, eta, zeta, N_face, nodes, nnode_face, n, gap, penalty, K_el, F_el)
  END SUBROUTINE PH_Elem_C3D15P_FormContactFaceCtr

  SUBROUTINE PH_Elem_C3D15P_FormFacePressure(coords, p, face_id, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    REAL(wp), INTENT(IN)  :: p
    INTEGER(i4), INTENT(IN)  :: face_id
    ...
    ELSE
      ...
      END DO
    END IF
  END SUBROUTINE PH_Elem_C3D15P_FormFacePressure

  SUBROUTINE PH_Elem_C3D15P_FormBodyForce(coords, bx, by, bz, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    ...
    DO ip = 1, 9
      ...
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D15P_FormBodyForce

  SUBROUTINE PH_Elem_C3D15P_FormPoreSource(coords, q_source, F_eq)
    REAL(wp), INTENT(IN)  :: coords(3, 15)
    ...
    DO ip = 1, 9
      ...
      IF (ABS(detJ) <= 1.0e-12_wp) CYCLE
      DO i = 1, 15
        F_eq(45+i) = F_eq(45+i) + N(i) * q_source * detJ * weights(ip)
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D15P_FormPoreSource

  SUBROUTINE PH_Elem_C3D15P_FormNodalForce(load_type, coords, val, face_id, F_eq)
    ...
    IF (load_type == PH_ELEM_LOAD_BODY) THEN
      CALL PH_Elem_C3D15P_FormBodyForce(coords, val(1), val(2), val(3), F_eq)
    ELSE IF (load_type == PH_ELEM_LOAD_FACE_P .AND. SIZE(val) >= 1) THEN
      CALL PH_Elem_C3D15P_FormFacePressure(coords, val(1), face_id, F_eq)
    ELSE IF (load_type == PH_ELEM_LOAD_PORE_SOURCE .AND. SIZE(val) >= 1) THEN
      CALL PH_Elem_C3D15P_FormPoreSource(coords, val(1), F_eq)
    END IF
  END SUBROUTINE PH_Elem_C3D15P_FormNodalForce

  SUBROUTINE PH_Elem_C3D15P_EvalPrincStress(sigma, principal)
  ...
  END SUBROUTINE PH_Elem_C3D15P_EvalPrincStress

  SUBROUTINE PH_Elem_C3D15P_EvalStressInvar(sigma, I1, J2, J3)
    REAL(wp), INTENT(IN)  :: sigma(6)
  ...
  END SUBROUTINE PH_Elem_C3D15P_EvalStressInvar

  SUBROUTINE PH_Elem_C3D15P_CollectIPVars(ip_stress, ip_strain, ip_pore, ip_peeq, n_ip, out_vars)
    ...
    END DO
  END SUBROUTINE PH_Elem_C3D15P_CollectIPVars

  SUBROUTINE PH_Elem_C3D15P_EvalVonMises(sigma, seq)
    REAL(wp), INTENT(IN)  :: sigma(6)
  ...
  END SUBROUTINE PH_Elem_C3D15P_EvalVonMises

  SUBROUTINE PH_Elem_C3D15P_GetExtrapMat(E)
    REAL(wp), INTENT(OUT) :: E(9, 9)
    E = ZERO
    E(1, 1) = ONE
    E(2, 2) = ONE
    E(3, 3) = ONE
    ...
    E(8, 8) = ONE
    E(9, 9) = ONE
  END SUBROUTINE PH_Elem_C3D15P_GetExtrapMat

  SUBROUTINE PH_Elem_C3D15P_MapToNode(ip_vars, weights, node_vars)
    ...
    node_vars = ZERO
    CALL PH_Elem_C3D15P_GetExtrapMat(E)
    n_comp = MIN(SIZE(ip_vars, 2), SIZE(node_vars, 2))
    DO ic = 1, n_comp
      DO i = 1, MIN(9, SIZE(ip_vars, 1))
        DO j = 1, 9
          node_vars(i, ic) = node_vars(i, ic) + E(i, j) * ip_vars(j, ic)
        END DO
      END DO
    END DO
  END SUBROUTINE PH_Elem_C3D15P_MapToNode

END MODULE PH_Elem_C3D15P