!===============================================================================
! MODULE: PH_Elem_R2D2Defn
! LAYER:  L4_PH
! DOMAIN: Element/Special
! ROLE:   Def
! BRIEF:  R2D2 ï¿?2-node 2D rigid element
!===============================================================================
MODULE PH_Elem_R2D2_Def
...
! > Theory: Internal UFC architecture spec §1 (see UFC_ .md) | Last verified: 2026-02-14
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: UF_Elem_R2D2_Calc
  PUBLIC :: PH_ELEM_R2D2_NNODE
  PUBLIC :: PH_ELEM_R2D2_NDOF

  INTEGER(i4), PARAMETER :: PH_ELEM_R2D2_NNODE = 2_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_R2D2_NDOF = 4_i4

CONTAINS

!===============================================================================
! Subroutine: UF_Elem_R2D2_Calc
! Purpose: Calc function for R2D2 rigid element (RT_Elem_Core interface)
!===============================================================================
  SUBROUTINE UF_Elem_R2D2_Calc(ElemType, Formul, Ctx, state_in, &
                                 Mat, state_out, flags)
    ...
    IF (nNode /= 2) THEN
      ...
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_R2D2_Calc: expected 2 nodes')
    ...
    IF (.NOT. ALLOCATED(Ctx%coords_ref)) THEN
      ...
      flags%failed = .TRUE.
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_R2D2_Calc: coords_ref not allocated')
      state_out%failed = flags%failed
      state_out%stableDt = flags%stableDt
      RETURN
    END IF
  ...
  END SUBROUTINE UF_Elem_R2D2_Calc

END MODULE PH_Elem_R2D2_Def