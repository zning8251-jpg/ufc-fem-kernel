!===============================================================================
! MODULE: PH_Elem_R3D3Defn
! LAYER:  L4_PH
! DOMAIN: Element/Special
! ROLE:   Def
! BRIEF:  R3D3 3-node 3D rigid element
!===============================================================================
MODULE PH_Elem_R3D3_Def
...
! > Theory: Internal UFC architecture spec §1 (see UFC_ .md) | Last verified: 2026-02-14
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: UF_Elem_R3D3_Calc
  PUBLIC :: PH_ELEM_R3D3_NNODE
  PUBLIC :: PH_ELEM_R3D3_NDOF

  INTEGER(i4), PARAMETER :: PH_ELEM_R3D3_NNODE = 3_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_R3D3_NDOF = 9_i4

CONTAINS

!===============================================================================
! Subroutine: UF_Elem_R3D3_Calc
! Purpose: Calc function for R3D3 rigid element (RT_Elem_Core interface)
!===============================================================================
  SUBROUTINE UF_Elem_R3D3_Calc(ElemType, Formul, Ctx, state_in, &
                                 Mat, state_out, flags)
    ...
    IF (nNode /= 3) THEN
      ...
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_R3D3_Calc: expected 3 nodes')
    ...
    coords(1:3, 1:3) = Ctx%coords_ref(1:3, 1:3)

    ! Extract displacements
    IF (ALLOCATED(Ctx%disp_total)) THEN
      IF (SIZE(Ctx%disp_total, 2) >= 3) THEN
        u(1:3, 1:3) = Ctx%disp_total(1:3, 1:3)
      ELSE
        u = 0.0_wp
      END IF
    ELSE
  ...
  END SUBROUTINE UF_Elem_R3D3_Calc

END MODULE PH_Elem_R3D3_Def