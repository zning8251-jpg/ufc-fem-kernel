!===============================================================================
! MODULE: PH_Elem_COH3D8Defn
! LAYER:  L4_PH
! DOMAIN: Element/Special
! ROLE:   Def
! BRIEF:  COH3D8 ï¿?8-node 3D cohesive element
!===============================================================================
MODULE PH_Elem_COH3D8_Def
...
! > Theory: Internal UFC architecture spec §1 (see UFC_ .md) | Last verified: 2026-02-14
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: UF_Elem_COH3D8_Calc
  PUBLIC :: PH_ELEM_COH3D8_NNODE
  PUBLIC :: PH_ELEM_COH3D8_NDOF

  INTEGER(i4), PARAMETER :: PH_ELEM_COH3D8_NNODE = 8_i4
  INTEGER(i4), PARAMETER :: PH_ELEM_COH3D8_NDOF = 24_i4
...
CONTAINS
  ...
  FUNCTION ComputeCohesiveArea(coords, nNode, nDim) RESULT(area)
    ...
    IF (nDim == 3 .AND. nNode >= 4) THEN
      ! 3D: compute area from quadrilateral (first 4 nodes)
  ...
  SUBROUTINE UF_Elem_COH3D8_Calc(ElemType, Formul, Ctx, state_in, &
                                   Mat, state_out, flags)
    ...
    IF (nNode /= 8) THEN
      ...
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_COH3D8_Calc: expected 8 nodes')
    ...
    IF (.NOT. ALLOCATED(Ctx%coords_ref)) THEN
      ...
      flags%failed = .TRUE.
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_COH3D8_Calc: coords_ref not allocated')
      state_out%failed = flags%failed
      state_out%stableDt = flags%stableDt
      RETURN
    END IF

    coords(1:3, 1:8) = Ctx%coords_ref(1:3, 1:8)

    IF (ALLOCATED(Ctx%disp_total)) THEN
      IF (SIZE(Ctx%disp_total, 2) >= 8) THEN
        u(1:3, 1:8) = Ctx%disp_total(1:3, 1:8)
      ELSE
        u = 0.0_wp
      END IF
    ELSE
    ...
    area = ComputeCohesiveArea(coords, 8, 3)
    ...
    IF (flags%failed) THEN
      CALL init_error_status(flags%status, IF_STATUS_INVALID, &
        message='UF_Elem_COH3D8_Calc: cohesive traction fully degraded (d_n or d_s >= 1)')
    ELSE
  ...
  END SUBROUTINE UF_Elem_COH3D8_Calc
END MODULE PH_Elem_COH3D8_Def