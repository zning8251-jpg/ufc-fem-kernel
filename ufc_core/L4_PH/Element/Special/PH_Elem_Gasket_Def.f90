!===============================================================================
! MODULE: PH_Elem_GasketDefn
! LAYER:  L4_PH
! DOMAIN: Element/Special
! ROLE:   Def
! BRIEF:  Elem Gasket Defn module (auto-filled)
!===============================================================================
MODULE PH_Elem_Gasket_Def
...
! > Theory: Internal UFC architecture spec §1 (see UFC_ .md) | Last verified: 2026-02-14
  !! ===================================================================
  !! UniField-Core Gasket Element Definition Module ( ï¿?
  !!   LAYER: L4 (Element Library)
  !!   DOMAIN: Element/SPECIAL/Gasket
  !!   KIND: Core (Gasket element kernels & unified interface)
  !! 
  !! This module provides the main gasket element interface:
  !!   - UF_Elem_Gasket_Calc: Unified interface with struct-based parameters
  !!   - Internal dispatch to element-family-specific implementations
  !!
  !! Element types:
  !!   - GK2D2: 2-node 2D gasket element
  !!   - GK3D4: 4-node 3D gasket element
  !!
  !! Design Principles:
  !!   -  ï¿? This is the main module for GASKET family
  !!   - Unified struct-based interface for L5_RT layer stability
  !!   - Internal dispatch to GK2D2/GK3D4 specific implementations
  ...
  use PH_Elem_GK2D2_Definition, only: UF_Elem_GK2D2_Calc
  use PH_Elem_GK3D4_Definition, only: UF_Elem_GK3D4_Calc
  ...
  ! PUBLIC INTERFACES
  !=============================================================================
  public :: UF_Elem_Gasket_Calc
  public :: PH_Elem_Gasket_Material_Update_Routed
...
contains
  ...
  subroutine UF_Elem_Gasket_Calc(ElemType, Formul, Ctx, state_in, &
                                   Mat, state_out, flags)
    !! Unified interface for all gasket elements (GK2D2, GK3D4).
    ...
    if (index(ename, 'GK2D2') > 0) then
      call UF_Elem_GK2D2_Calc(ElemType, Formul, Ctx, state_in, &
                              Mat, state_out, flags)
    else if (index(ename, 'GK3D4') > 0) then
      call UF_Elem_GK3D4_Calc(ElemType, Formul, Ctx, state_in, &
                              Mat, state_out, flags)
    else
      ...
      if (ElemType%numNodes == 2 .and. ElemType%dim == 2) then
        ...
        call init_error_status(flags%status, IF_STATUS_INVALID, &
          message='UF_Elem_Gasket_Calc: unsupported gasket topology (expected GK2D2/GK3D4 match)')
        state_out%failed = flags%failed
        state_out%stableDt = flags%stableDt
      end if
    end if

  end subroutine UF_Elem_Gasket_Calc

  SUBROUTINE PH_Elem_Gasket_Material_Update_Routed(rt_ctx, mat_slot, K_g, h_0, &
                                                   p_max, status)
    USE IF_Mat_Dispatch_Def, ONLY: RT_Mat_Dispatch_Ctx
    USE PH_Mat_Def, ONLY: PH_Mat_Slot
    USE PH_Elem_MaterialRoute, ONLY: PH_Elem_MatRoute_GasketLinear
  ...
  END SUBROUTINE PH_Elem_Gasket_Material_Update_Routed
END MODULE PH_Elem_Gasket_Def
