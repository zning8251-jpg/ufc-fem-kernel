...
- [ ] 2.1 `UF_Hill_UMAT_Arg` + `UF_Hill_UMAT(arg)`；`UF_Hill_UMAT_Core` private
- [ ] 2.2 `PH_MatPLMEval` / `PH_MatPLM_Kernels` 挂接 Hill Arg
- [ ] 2.3 `PH_Mat_Barlat_Calc_Stress_Arg` + 单参入口；旧签名 private
- [ ] 2.4 MOD-001：Hill / Barlat 模块头
- [ ] 2.5 harness：discipline + guardian touched + `change-package validate --strict`
